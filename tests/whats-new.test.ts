...
describe("collectWhatsNew", () => {
  it("不含 after、含 through", () => {
    const blocks = collectWhatsNew("1.9.5", "1.9.6");
    expect(blocks.map((b) => b.version)).toEqual(["1.9.6"]);
  });

  it("首次 after=null 仍返回 through 及更早有文案的版本（调用方自己决定不弹）", () => {
    const blocks = collectWhatsNew(null, "1.9.6");
    expect(blocks.map((b) => b.version)).toContain("1.9.6");
    expect(blocks.map((b) => b.version)).toContain("1.9.5");
  });

  it("比 through 新的条目不收", () => {
    const blocks = collectWhatsNew("1.9.0", "1.9.2");
    expect(blocks.every((b) => b.version !== "1.9.6")).toBe(true);
...
describe("notesForCurrentVersion", () => {
  it("有本版条目就只返回本版", () => {
    const blocks = notesForCurrentVersion("1.9.6");
    expect(blocks).toHaveLength(1);
    expect(blocks[0]?.version).toBe("1.9.6");
  });
