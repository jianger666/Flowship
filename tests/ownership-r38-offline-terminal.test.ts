...
const applyHydratedWatchCatch = (
  ...
  if (decision.action === "terminate_deleted") {
    ...
    return {
      stop: true,
      delayMs: null,
      nextFailures: state.consecutiveFailures,
  ...
  return {
    ...
    nextFailures: decision.nextConsecutiveFailures,
  ...
  it(
    ...
    async () => {
      ...
      const first = applyHydratedWatchCatch(
        id,
        err,
        { consecutiveFailures: 0, unavailableNotified: false },
        handlers,
      );
...
describe("R38-② 503 证据 I/O 故障不 commit deleted", () => {
  ...
  it("真实 watch：注入 journal EIO → 503、client 不 commit", async () => {
    ...
    applyHydratedWatchCatch(
      ...
      { consecutiveFailures: 2, unavailableNotified: false },
      {
        onTaskDeleted: () => {
          deletedCalls += 1;
...
describe("R38-③ 503 持续重试：7 次后仍能恢复", () => {
  it("fake timers：7×503 提示一次且不终止，随后成功恢复", async () => {
    ...
    const runLoop = async () => {
      ...
      for (const outcome of outcomes) {
        ...
        const step = applyHydratedWatchCatch(
          id,
          new ApiRequestError("unavailable", 503),
          { consecutiveFailures, unavailableNotified },
          {
            onTaskDeleted: () => {
              deletedCalls += 1;
            },
        ...
        consecutiveFailures = step.nextFailures;
    ...
    if (at5.action === "retry") {
      ...
      expect(at5.nextConsecutiveFailures).toBe(WATCH_MAX_TRANSIENT_FAILURES);
    }
    ...
    if (at6.action === "retry") {
      ...
      // nextFailures=7 → min(7*1500, cap)=10500；更高次数才触顶
  ...
  it("普通网络错仍保留 6 次上限终止（与 503 持续重试对照）", () => {
    ...
    for (let i = 0; i < WATCH_MAX_TRANSIENT_FAILURES; i++) {
      ...
      if (d.action === "retry") {
        failures = d.nextConsecutiveFailures;
        notified = d.nextUnavailableNotified;
      }
    }
    expect(stop).toBe(true);
    expect(failures).toBe(WATCH_MAX_TRANSIENT_FAILURES - 1);
  });
});
...
describe("R38-④ 410 立即 sticky 终止", () => {
  it("410 → terminate_deleted；commit 后详情/侧栏移除", () => {
    ...
    const step = applyHydratedWatchCatch(
      ...
      { consecutiveFailures: 3, unavailableNotified: false },
      {
