...
### v1.9.3（2026-08-26）流式吐字丝滑化

- **流式合帧**：服务端 `assistant_delta` 按 70ms / 240B 合帧；客户端 rAF 合帧 + 自适应平滑追赶。ChatView 与任务详情页共用 `use-smooth-streaming`。
...
- **任务详情头部重构**：头部收敛为「标题 / 操作行 + 任务级入口行 + 按仓项目卡片」；上下文文档改名任务上下文、MCP servers 改名 MCP；需求群 / 任务文件夹并入任务级入口行（ghost 按钮、竖分隔线区分弹窗触发与执行型按钮）；未分批按钮不再展示（半兼容、数据保留）；单仓 / 多仓统一短名展示。
- **每仓分支展示 + 切换**：项目卡片内分支 chip（图标 + 等宽名、tooltip 只显示分支名），点击弹层切换并带警告「切到非任务相关分支可能会有一些未知问题」；worktree 未就绪时弹层只提示、禁切（后端 POST 兜底 400）；分支下拉提成统一 `BranchSwitcher`（chat=select / task=chip 两种形态）。
- **产物区 / 操作区打磨**：产物文件名并入 ArtifactPanel toolbar（去掉独立行、11px 弱化、hover 看全名）；timeline chip 与主操作按钮去掉冗余 tooltip（保留纯图标按钮的提示）；拖拽分隔柄修掉点击后沿全高细条出现的浏览器默认黑 outline；预览 tooltip 只显示启动命令。
- **门禁**：typecheck / lint 干净；vitest 全量通过（依赖本地端口 / 子进程的用例在沙箱外复跑通过）；standalone test 包构建通过。
...
- **仓库改名 fe-ai-flow → Flowship**（github.com/jianger666/Flowship）：GitHub 旧名自动重定向、存量 mac/win 自更新链不受影响；壳内 3 处 URL + electron-builder publish.repo + README 已改；appId / 安装包文件名 / 数据目录名不动（改了断更新链/丢数据）；⚠️ 永远别再建名为 fe-ai-flow 的新仓（顶掉重定向）。（后记：改名当天暴露 mac 更新链 `redirect:"manual"` 抠不到改名重定向后的 tag、存量客户端更新断——仓名已回滚 fe-ai-flow，重定向安全修复随 v1.1.14 发、等全员升级后再改名，见 v1.1.14 段。）
