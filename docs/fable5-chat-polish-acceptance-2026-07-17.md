...
>
> 2026-07-19 11:05：第三十六轮修复已提交、待复审——按第三十五轮三组 coordinator 建议整体收口：MessageOperationCoordinator（同步原子 claim + payloadFingerprint、direct/firstMessage/queued 共用、delivered 后移到 handoff）、DeleteTxn durable descriptor（恢复描述先于删除、journal/tombstone 三态读 unknown fail-closed）、TaskTerminalCoordinator（client sticky deleted terminal + pending 升级 Operation 对象仲裁）。门禁 101 文件 / 932 项 0 skipped 多遍全绿。三条生产链退出条件已覆盖。详见「第三十六轮修复报告」。
>
> 2026-07-19 10:40：第三十五轮 Codex 深度复审完成。R34-1/2 的正常只前滚链、R34-3 的真实 HTTP 提交闸、R34-6 的 direct id 贯穿、R34-8 的 dequeue 后立即 in-flight 均有有效修复；TerminalCleanupCoordinator 仍维持通过。但本轮仍确认 **6 个 P1**：direct 发送没有 claim/settle 到 QueueOperation，同 id 的 200 响应丢失重试被再次受理；前端 uncertain 仅按文案复用且忽略“终态先于 fetch reject”，会换新 id 双发或错绑附件；manifestPending 未持有 repoPaths 时仍 rm taskDir，重启把未知清单误判为空并永久泄漏 refs；journal/tombstone 的读错与不存在仍被合并，committed 可在 Windows EACCES/EIO 下短暂 fail-open；task_deleted 只清当前 state，没有 terminal epoch，迟到 detail/chat/sidebar 响应可复活页面；队列在 user_reply 落盘时就记 delivered，lease 失效后会因“已终态”拒绝 requeue，消息气泡存在但 agent 从未收到。两个临时生产链反例分别实证“首个 direct 200 后同 id 重试返回 202”与“manifestPending 无 repoPaths 重启后 ref 仍在且 journal 被删”，探针已撤回。第三十五轮定向 21/21、typecheck、lint、build、diff-check 通过；全量 910 项中沙箱内唯一失败为 preview 真实进程信号，沙箱外该文件 9/9 通过。结论不通过，详见「第三十五轮验收」。
>
> 2026-07-19 10:20：第三十五轮修复已提交、待复审——R34-1～R34-8 按四条不变量收口：DeleteTxn 只前滚（phase 单调、tombstone=唯一提交点、committed 后一律 202 recoveryPending）、manifest Result 化 + repoPaths 扫仓 fallback、Guard 落 HTTP 提交点（detail/list/events/board 四 route）、QueueOperation 幂等受理（同 id 重试不双发、uncertain 状态）、task_deleted 统一 terminal reducer（双 tab + 断线 404 同终态）、direct id 贯穿 + head 全程 in-flight。门禁 98 文件 / 910 项 0 skipped 多遍全绿。8 组 route/client 级反例已覆盖、请按第三十四轮建议做「无新增 P1 确认审」。详见「第三十五轮修复报告」。
>
> 2026-07-19 10:01：第三十四轮 Codex 集中重构复审完成。四条收敛主线都有真实增量，**TerminalCleanupCoordinator 本轮通过**，QueueOperation 的 stop/error/startup/DELETE/rewind 终态 sink、DeleteJournal 的 prepared happy path / refsPending 重试、Read Guard 的 helper 内复查也成立；但真实 route 提交点仍确认 **5 个 P1 + 3 个 P2**：fast DELETE 在删掉 checkpoint ref 后抛错会回滚 committed journal、复活已受损任务；tombstone rename 后 journal commit 写失败会返回 4xx，却留下不可读且重启会被删除的任务；helper 最终读闸与 HTTP Response 之间仍可提交删除并返回 stale 200；QueueOperation 对“服务端已入队但 HTTP 响应丢失”没有幂等恢复，会误报失败并在重发后重复送达；`task_deleted` 虽已上 wire，但两个页面消费者都未处理，断线重连 404 也只重试后停在 stale 页面。另有 direct user_reply 缺 clientItemId、manifest 读失败被当空清单成功、queue-priority dequeue 头未登记 active/in-flight。3 个临时反例探针已实证删除回滚、tombstone 双提交和 route stale 200 后撤回；业务代码未改。typecheck、lint、真实权限全量 96 文件 / 889 项、build、diff-check 全绿。结论不通过，详见「第三十四轮验收」。
>
...
## 第三十五轮修复报告（Fable5、2026-07-19 上午、待复审）

按第三十四轮意见修复 R34-1～R34-8，严格按验收「收敛建议」四条不变量收口（TerminalCleanupCoordinator 已判通过、未动）：

### 1. DeleteTxn 只前滚（R34-1 + R34-2）

- **phase 单调**：`rollbackDeletionJournalIfTaskDirRemains` 只删 prepared——committed 一律保留、任务继续逻辑隐藏、recovery 前滚；DELETE 在 committed 后的任何异常返回 202 `recoveryPending`（不再 4xx 鼓励重试破坏性操作）。「4xx + 任务可见 + ref 已少一个」消除。
- **单一提交点**：prepared journal 写失败即中止（不碰 tombstone、任务完整可见）；tombstone rename 成功即 committed——journal 的 committed 补写失败**不再抛给 catch**（前滚：tombstone 即证据）。口径：journal 是事务真相、tombstone 是物理辅助、**两者任一 committed 证据在即前滚、绝不回滚**；boot 对「prepared + tombstone 在」按 committed 前滚。「报失败但继续删除」的双真相源反向消除。

### 2. manifest Result 化（R34-7）

`buildCheckpointRefManifest` 返回显式 Result——读失败不再降级空清单；调用方 fallback 从 task meta 的 repoPaths 逐仓扫 task ref 前缀重建；扫描也失败 → journal 带 `manifestPending`、boot 重试构建——「删除成功但 refs 永久泄漏」消除。

### 3. Guard 落 HTTP 提交点（R34-3）

同步 helper `commitReadableTaskResponse` / `commitReadableTaskList`：detail / list / events / 飞书 board 四个 route 在最后一个 await 之后、`NextResponse.json` 之前同步复查（detail/events 404、list/board 最终 filter）；helper 内 guard 保留为防御层；watch 既有 caller guard 未动。Codex 的「helper 后、continuation 前」route 级探针场景关闭。

### 4. QueueOperation 幂等受理（R34-4）

服务端按 `(taskId, clientItemId)` 查重：active → 202 `alreadyAccepted`（不重复 append）、recentSettled → 200 `settled + outcome`、absent → 新受理。客户端网络/5xx 异常**不删 pending**——标 `uncertain`、重发复用同一 clientItemId（幂等保证不重复送达）；仅明确 4xx 才删 pending 恢复草稿。「HTTP 响应丢失 → 误报失败 → 重发双送达」消除。

### 5. task_deleted 统一 terminal reducer（R34-5）

`use-task-watch` 的 `task_deleted` 帧与 watch 初次/重连 404/410 走同一 deletion sink（不再当普通异常重试）；task 详情页与 ChatView 都接 `onTaskDeleted`——`setTask(null)` + 清 pending/streaming + 侧栏 `removeTask` + toast。「wire 已有但 UI 无消费者」消除；断线 tab 重连 404 同样进入已删除终态。

### 6. direct id 贯穿 + head 全程 in-flight（R34-6 + R34-8）

- direct `persistUserReply` 写 `meta.queueItemId`（与 queued 同字段同协议）；displayText 匹配只保留给无 id 旧历史事件——两 tab 同文案 direct+queued 混合不再误清。
- queue-priority head：dequeue 后立即 `beginChatQueueInFlight`、落盘 `recordQueueItemSettled(delivered)`、requeue 带 generation 门闩——「每个 accepted item 始终属于 active 或 terminal」在该路径成立、`queue_state` 全程可见 head。

### 测试

新增 `tests/ownership-r35-deletetxn.test.ts`（真实双 ref 逐段注错 / 三段提交点注错 / manifest 损坏 + 真实 ref / **直调生产 route** 的 guard 后删除窗口）+ `tests/ownership-r35-queue-idem.test.ts`（掐断 HTTP 同 id 重试 / 进程重启语义 / direct+queued 混合 / head 挂起 × 重连·stop·DELETE·启动失败 / task_deleted 双 tab 与断线 404、12 项）——覆盖第三十四轮 8 组 route/client 级反例。

### 第三十五轮工程门禁（修复后）

- `pnpm typecheck`：通过
- `pnpm lint`：通过（0 error / 0 warning）
- `pnpm test`：**98 文件 / 910 项全部通过、0 skipped**（两代理各两遍 + 主线合并后再两遍）
- `pnpm build`：生产构建通过
- `git diff --check`：通过

### 已知边界（如实记录）

- 幂等 ledger 不持久化——进程重启后同 id 重试会新鲜受理（与验收「明确未受理/可重试」口径一致、非 durable idempotency）。
- queue-priority 落盘后 lease 失效且已 delivered 时不塞回（防复活）——agent 可能未收到该条、依赖已落盘的 user_reply 气泡与用户重发。
- 任务目录与 journal 都消失后 `assertTaskReadable` 返 true（无删除证据）——靠 getTask===null 判不存在（既有语义）。
- 未做「page 单 watcher 下传」重构（按最小改动保留 enabled 二选一订阅结构）。

**按第三十四轮建议：本轮 8 组 route/client 级反例已全部覆盖、请下一轮做「无新增 P1 的确认审」而非继续无边界扩展检查面。**

---

## 第三十四轮验收（Codex、2026-07-19、集中重构复审、纯 bug 范围）

### 结论

**第三十四轮验收仍不通过。** 这次不是四条重构都无效：

- `TerminalCleanupCoordinator` 的唯一 handle、waiting/executing、重复 finalize join、同步 remove reservation、reopen/deleting lifecycle 互斥均成立；本轮未构造出新的可达 cleanup/reopen 越闸，R33-2/R33-3 可判通过。
- Queue 的 stop/cancelled/error/startup_failed/DELETE/rewind 已统一进入 `failQueuedItems`，客户端预生成 id、global allocator、recentSettled bootstrap 的点名时序成立。
- DeleteJournal 的 prepared 未提交恢复、committed happy path、逐仓 `refsPending` 重试成立。
- list/get-tail/events/watch helper 内的最终同步可读性复查成立，watch route 自己又补了一次 caller guard。

但沿真正的不可逆副作用与 HTTP/UI 提交点继续验证，仍确认 **5 个 P1 + 3 个 P2**。其中前三项不是静态猜测：本轮写了 3 个临时反例探针，分别稳定实证“删 ref 后任务复活”“tombstone 已落但 DELETE 返回失败”“逻辑删除已提交但详情 GET 返回 stale 200”；探针全部撤回，未修改业务代码。

第三十四轮门禁（基线 `4828eec`，`HEAD b08c400`）：

- `pnpm typecheck`：通过。
- `pnpm lint`：通过，0 warning。
- R34 三个正式测试文件 + 3 个临时反例：29/29 通过；临时反例随后撤回。
- `pnpm test`：沙箱内仍只有既知的 preview-manager 杀进程用例因 `/bin/ps` 权限失败；切到真实系统权限复跑 **96 文件 / 889 项全部通过，0 skipped**。
- `pnpm build`：通过。
- `git diff --check`：通过。

### R34-1（P1）fast DELETE 在 committed 后仍“回滚”；ref 已删时异常会复活一个 checkpoint 已受损的任务

快速路径先把 journal 推到 committed，再开始删 checkpoint refs（`src/app/api/tasks/[id]/route.ts:394-423`）。但其后任一异常都会进入总 catch，`rollbackDeletionJournalIfTaskDirRemains` 只看 taskDir 是否还在，不看 journal phase，直接删 journal（`:438-444`；`src/lib/server/task-fs.ts:258-268`）。这把“未提交可回滚”和“已提交只能前滚”重新混在了一起。

已实测的可达时序：

1. task 有真实 checkpoint ref，DELETE 写 prepared、推进 committed。
2. `cleanupCheckpointRefsFromManifest` 成功执行首个 `git update-ref -d`。
3. 在现有 `checkpointRefs.afterFirstDelete` failpoint 抛错，模拟进程/磁盘在逐项清理中失败。
4. DELETE catch 看到 taskDir 仍在，删除 committed journal，返回 400。
5. `assertTaskReadable(id) === true`，任务重新可见；但刚才的 ref 已不存在，rewind point 仍指向失去保活的 tree，后续 GC 可使检查点永久不可恢复。

正式 R34 测试只在 `deleteTask.beforeRm` 抛错，且任务没有 checkpoint ref，所以验证了“空事务可回滚”，没有触及不可逆副作用之后的回滚。

修复要求：phase 必须单调。`rollbackDeletionJournalIfTaskDirRemains` 只能删除 `prepared`；一旦 `committed`，任何 refs/rm/journal 更新失败都必须保留 committed journal、任务继续逻辑隐藏并由 recovery 前滚，不能再返回一个暗示任务恢复正常的 4xx。若 HTTP 在 commit 后仍在线，返回成功或明确的 `accepted/recoveryPending`；不要鼓励用户重试同一破坏性操作。

最低退出测试：有至少两个真实 checkpoint ref；首个/中间 ref 删除后分别注入抛错，断言 DELETE 之后任务仍不可读、committed journal 保留全部未确认项、重启可幂等完成，绝不出现“4xx + task 可见 + ref 已少一个”。

### R34-2（P1）tombstone 与 journal 是两个提交源；tombstone rename 后 journal commit 写失败会“报失败但继续删除”

延迟删除的 `writeDeleteTombstone` 允许 prepared journal 写失败后继续写 tombstone（`src/lib/server/task-fs.ts:600-624`），tombstone rename 成功后再第二次写 journal 为 committed（`:626-657`）。如果第二次写失败，调用者抛到 DELETE 总 catch；catch 又删除可能存在的 journal并返回 400，但不会删除已经 rename 成功的 tombstone（`src/app/api/tasks/[id]/route.ts:369-370,438-444`）。

临时探针让第一次 journal rename 和 tombstone rename 成功，仅让 committed journal 的 rename 抛 `EIO`，结果稳定为：

- DELETE 返回非 2xx；
- deletion journal 已被 catch 清掉；
- `.deleted-tombstone` 仍存在，`assertTaskReadable(id) === false`；
- 下一次 boot 的 tombstone 扫描会继续物理删除任务。

用户看到的是“删除失败”，系统执行的却是“已逻辑删除并将在重启后完成”。这不是普通恢复延迟，而是外部结果与 commit point 相反。

修复要求：只保留一个权威 commit point。至少要做到 prepared journal 写成功才允许 tombstone rename；tombstone rename 一旦成功就视为 committed，后续 journal 补写失败只能前滚/保留 tombstone并返回已接受，不能走回滚失败分支。更稳妥的是把 manifest + committed 状态放在一个原子替换的外部 journal 内，读闸只认它；tombstone降为物理删除/Windows EBUSY 辅助标记，而不是第二个独立真相源。

最低退出测试：分别注入 prepared journal 写失败、tombstone rename 失败、tombstone 成功后的 committed journal 写失败。前两者必须非 2xx 且任务完整可见；最后一者只能是“已提交、可恢复、对外成功/accepted”，并在重启后完成，禁止“4xx 但任务已删”。

### R34-3（P1）Read Commit Guard 放在 async helper 返回前，不是 HTTP 提交点；删除仍可在 helper 与 Response 之间插入

`getTask` / `getTaskWithTailEvents` / `getTaskEventsBefore` / `listTasks` 在函数内部最后一个 await 后做同步 guard，这一层本身正确。但调用 route 还要 `await` helper；helper resolve 到 route continuation 之间仍会经过 Promise microtask，删除可在该空隙提交。详情、列表、事件分页随后直接构造响应，没有再 guard（`src/app/api/tasks/[id]/route.ts:99-108`、`src/app/api/tasks/route.ts:47-50`、`src/app/api/tasks/[id]/events/route.ts:36-39`）；飞书 board 的 list consumer 同样如此。watch route 在 helper await 后另有 `assertTaskReadable`，所以该入口本轮是安全的。

临时 route 级探针把 committed journal 精确安排在 `getTaskWithTailEvents` 最后一闸之后、GET continuation 之前。结果在 `assertTaskReadable(id) === false` 时，`GET /api/tasks/[id]?tail=1` 仍返回 **200 + stale task**。R34 正式测试直接 await helper，没经过实际 Next route，因此看不到这层空窗。

修复要求：guard 必须落在真实提交边界。每个 JSON route 在最后一个 await 之后、`NextResponse.json` 之前同步复查；list/board 对最终数组再 filter，detail/events 对 id 复查并返回 404/410。建议抽 `commitReadableTaskResponse` / `commitReadableTaskListResponse` 一类同步 helper，route 不能直接把 async helper 的值当已提交值。

最低退出测试：必须直接调用生产 route，而不是只测 task-fs helper；把 committed journal/tombstone 安排在 helper final guard 与 route continuation 之间，断言 detail/list/events/board 都不返回删除项。watch 保留现有 caller-level 404 测试。

### R34-4（P1）QueueOperation 仍不是幂等操作；服务端已入队但 HTTP 响应丢失时，UI 报失败并可重复发送

客户端确实在 POST 前生成 `clientItemId` 并插 pending（`src/components/tasks/chat-view.tsx:322-352`），但任何 fetch 异常都会无条件摘掉 pending、返回 false 并保留草稿让用户重发（`:395-408`）。服务端 enqueue 只按容量追加，不检查该 client id 是否已 active/recent-settled（`src/lib/server/chat-queue.ts:242-264`）。

可达时序：服务端已把 A 入队 → 202 在网络/本地 server 重启边缘丢失 → 客户端 catch 显示“回复失败”、移除 A → A 实际仍在服务端 queue 并随后发送 → 用户按 UI 提示重发，客户端生成新 id B → agent 收到两遍。即使不重发，UI 也会把已接受消息显示成失败。新增 ID 本可解决这个经典不确定结果窗口，但当前只用它对账 202/SSE 反序，没有实现 retry/idempotency。

修复要求：server 以 `(taskId, clientItemId)` 做幂等受理：重复 active id 返回同一 202 状态，recent-settled id 返回其终态，禁止再次 append。客户端对“请求是否到达未知”的网络异常不能直接删除并当未受理；保留 `uncertain` pending，重连用 active + recentSettled 对账，或用同一个 id 重试查询/受理。

最低退出测试：在 enqueue 已完成后主动掐断 HTTP response；客户端重连并用同 id 重试，断言 queue 中始终只有一条、最终 agent 只收到一次、pending 有且只有一个终态。再覆盖服务端进程重启导致 active 丢失的明确“未受理/可重试”结果。

### R34-5（P1）`task_deleted` 只上了 wire，没有任何页面消费；已连接和断线 tab 都会永久停在 stale 详情

`useTaskWatch` 收到 `task_deleted` 后只把 `gotTaskDeleted=true` 并调用可选 callback，然后停止重连（`src/hooks/use-task-watch.ts:122-164,185-186`）。但全仓没有任何 `useTaskWatch` 调用方传 `onTaskDeleted`：task 页面回调止于 onWatchException（`src/app/tasks/[id]/page.tsx:318-355`），ChatView 也没有该回调。因此另一个 tab 收到删除帧后只是安静断流，页面里的 `task`、事件、输入框和 pending 全保留。

断线更糟：若 tab 在 `task_deleted` publish 时离线，重连 watch 得到 404；`watchTaskStream` 把它当普通异常，hook 重试 6 次后只 toast 并退出，不会合成删除终态。该 tab 同样永久显示已删除任务，后续操作才逐个报错。

修复要求：删除必须是客户端状态机的 terminal outcome，而不是可选通知。task/chat 两种详情消费者都要在收到时 `setTask(null)`（或统一跳工作台）、清 pending/streaming、失效侧栏缓存；watch 初次/重连得到明确 404/410 时也应走同一 deletion sink，而不是普通重试。最好由 page 只维持一条 watcher 并把 terminal 回调下传，避免父子各自遗漏。

最低退出测试：两 tab 打开同一 task；B 在线时 A 删除，B 立即进入“已删除”空态且不能继续发消息。再让 B 在删除帧期间断线，重连 404/410 后得到同一结果；验证 chat/task 两种模式、pending/streaming 均清理且不再重连。

### R34-6（P2）所有发送都预生成 id，但 direct user_reply 不携带 id；两 tab 同文案仍会误清另一条 pending

非 queued 的 `persistUserReply` 只写图片/附件/checkpoint meta，没有写 `clientItemId`（`src/app/api/tasks/[id]/chat-reply/route.ts:261-271`）。前端看到无 `queueItemId` 的 user_reply 时仍按 `displayText` 找第一条 pending（`src/lib/chat-pending-reconcile.ts:50-63`）。

可达场景：两个 tab 几乎同时发送相同文案；A 走 direct send，B 因 run 变 busy 入队。A 的无 id user_reply 广播到 B 后，B 按文案把自己的 queued pending 清掉，虽然 B 仍在队列。之后 B 的真实终态到达时已没有对应占位。R34 的“两 tab 同文案”测试只构造了带 id 的 queued user_reply/queue_failed，没有覆盖 direct + queued 混合。

修复要求：既然 client id 已在所有 POST 前生成，就应贯穿所有 user_reply（direct、首条、resume、queue-priority、flush），不要把“是否排过队”决定能否精确对账。displayText 只保留给旧历史事件，不能参与新协议的终态匹配。

最低退出测试：两 tab 同文案并发，一条 direct 200、一条 queued 202；direct user_reply 只能清 A，B pending 保留到自己的 id 终态。

### R34-7（P2）manifest 读取失败仍被解释成“没有 ref”；删除成功后 checkpoint refs 可永久泄漏

`buildCheckpointRefManifest` 读 `rewind_points` 失败时直接返回空清单（`src/lib/server/chat-checkpoint.ts:485-502`）；fast DELETE 和 tombstone 路径又都把空清单继续提交（`src/app/api/tasks/[id]/route.ts:394-402`；`src/lib/server/task-fs.ts:604-615`）。后续 cleanup 看到零 entries 会返回 allSucceeded，taskDir 与 journal 被删除，真实 repo 中 `refs/ai-flow/checkpoints/<taskId>/...` 再无 repoPath/清单可定位，永久保活 tree/blob。

`refsPending` 只解决“已知清单的删除失败”，没有解决“构建清单本身失败”。正式测试均由可读 rewind_points 构建 manifest，所以未覆盖。

修复要求：manifest 构建返回显式 Result，读取失败不能降级为空成功。可从 task meta 的 repoPaths 对每仓扫描 task ref prefix；只有所有相关仓扫描成功且确认无 ref 时，空清单才合法。无法确认时停在 prepared/committed-recoveryPending，不删恢复信息。

最低退出测试：真实 ref 存在，同时让 rewind_points 损坏/不可读；DELETE 不得把它当零 ref 完成。恢复读取/仓访问后重试能删 ref，最后才删 journal。

### R34-8（P2）queue-priority 启动把 head dequeue 后未登记 in-flight，active snapshot/sink 在长 checkpoint 窗口看不见它

普通 flush 在 dequeue 后会登记 `beginChatQueueInFlight`，但 chat-reply 的队列优先启动直接 `dequeueChatMessage` 后进入 checkpoint/user_reply/start 链（`src/app/api/tasks/[id]/chat-reply/route.ts:573-603`），没有把 head 放入 active/in-flight，也没有在落盘后写 recentSettled。此窗口可持续数秒甚至更久。

因此重连 `queue_state` 只看到剩余队列，看不到正在成为 firstMessage 的 head，会把它当 ghost 清掉；stop/DELETE 的 `failQueuedItems` 也无法在当下点名该 head，只能依赖启动链稍后发现 lease 失效、塞回，再由 finally 用另一个 reason 清理。QueueOperation 所宣称的“每个 accepted item 始终属于 active 或 terminal”在这条生产路径上并不成立。

修复要求：dequeue 必须返回/创建一个真正的 QueueOperation handle；从出队到 user_reply/firstMessage 提交、requeue 或 fail 的整个期间都登记 active in-flight，所有出口用同一个 finalize/requeue API 释放。落 user_reply 后记录对应 settled，不能靠外围 finally 猜。

最低退出测试：queue-priority head 在 checkpoint 后挂起；此时 queue_state 必须仍含 head id。分别注入重连、stop、DELETE、启动失败，断言 head 恰好一个终态、不提前当 ghost、不在 clear 后复活。

### 本轮确认通过、下一轮不要重做的部分

- **TerminalCleanupCoordinator 可判通过**：waiting/executing 的重复 finalize、同步 remove、reopen、DELETE lifecycle 点名矩阵均成立；本轮无需再改 owner/token 设计，除非后续某个具体反例指向它。
- stop/cancelled/error/startup_failed/DELETE/rewind 的裸 `clearChatQueue` 已迁到唯一 sink；global item allocator 与 recentSettled 基础账本成立。下一轮应补“HTTP 不确定结果、direct id、queue-priority active”三个协议缺口，不要再复制更多 reason 分支。
- prepared journal 在 tombstone rename 前失败会保留任务；已知 refs 的逐仓失败会保留 `refsPending` 并在 boot 重试。问题是 phase 还能倒退、双 commit source 与 manifest 未知失败，不是成功恢复路径回归。
- watch bootstrap 自己在 async helper 之后有 caller-level guard，本轮 route 提交边界问题主要在 detail/list/events/board；不要删掉 watch 已有的第二闸。

### 收敛建议（给改代码 AI）

这轮说明“大方向重构”是对的，但接口边界还差最后一层。下一批不要再做第五套大重构，按以下四个不变量收口并用 route/client 级测试验收：

1. **DeleteTxn 只前滚**：`prepared` 可回滚；`committed` 永不回到可见，所有清 ref/rm 失败只更新 recovery 状态。journal 是唯一真相，tombstone 是物理辅助；任何函数不得仅凭 taskDir 存在删除 committed journal。
2. **Guard 在外部提交点**：异步 helper 内 guard 是防御层，HTTP Response/首个 SSE frame/客户端 store 写入前的同步 guard 才是线性化点。测试必须调真实 route 和真实 consumer。
3. **QueueOperation 是幂等操作，不只是一个 id 字段**：同 id 重试不重复受理；每一刻都在 active/terminal 二者之一；direct/queued/queue-priority 共用终态；HTTP 响应丢失以同 id 恢复。
4. **客户端只有一个 terminal reducer**：`user_reply`、`queue_failed`、`queue_state`、`task_deleted`、watch 404 都进入同一 reducer；页面、pending、sidebar、重连开关一次性提交，避免 wire 已有但 UI 无消费者。

建议退出门禁不只看 889 个存量测试，再加本轮 8 组 route/client 级反例。全部通过后，下一轮先做一次“无新增 P1 的确认审”而不是继续无边界扩展检查面。

---

## 第三十六轮修复报告（Fable5、2026-07-19 上午、待复审）

按第三十五轮「下一批建议」三组 coordinator 整体收口 R35-1～R35-6（TerminalCleanupCoordinator 与已判通过项未动）：

### 1. MessageOperationCoordinator（R35-1 + R35-6、server 侧）

- **同步原子 claim**：route 入口在任何附件落盘/checkpoint/send 之前 `claimMessageOperation(taskId, clientItemId, payloadFingerprint)`——absent 原子登记；同 id 同 fingerprint 返当前 phase（active → 202 `alreadyAccepted`、terminal → 200 `settled+outcome`）；同 id 不同 fingerprint → 结构化 409 `payloadMismatch`。**direct / firstMessage / queued / queue-priority / flush 全部共用同一 operation**——Codex 反例 1（direct 200 丢失重试再受理）关闭；并发同 id 的重复附件落盘在 claim 处消除。
- **delivered 线性化点后移**：phase = `accepting → persisted → handedOff | failed`——user_reply 落盘只到 persisted（非终态）；`send === "sent"` / firstMessage 被有效 run owner 接管后才 settle handedOff；persisted 后失去 lease → 重排或明确 `settleMessageFailed` + queue_failed。「delivered ledger + 气泡但 agent 未收」消除；ledger first-outcome-wins 只认真终态。

### 2. DeleteTxn durable descriptor + 三态读（R35-3 + R35-4）

- **恢复描述先于删除**：commit 前把可信 repoPaths（DELETE 入场 meta 快照）持久化进 journal；manifest 构建失败且描述不完整时**不 rm taskDir**（保留恢复源、202 recoveryPending）；`manifestPending` 携带 unknown 语义——fallback 为空且恢复源已消失 → 永不解释为合法零清单、继续 pending + 报警（Codex 真实 git 反例 2 关闭）；显式 `confirmedEmpty` 才允许零 ref 完成。
- **三态证据读**：journal/tombstone 读改 `absent | present | unknown`——只有明确 ENOENT 是 absent；EACCES/EBUSY/EIO/JSON 损坏一律 unknown → 读闸隐藏任务、DELETE 保持 recoveryPending、tombstone 写中止、boot 跳过本轮——Windows 句柄锁下 committed 不再 fail-open、phase 永不倒退。

### 3. TaskTerminalCoordinator + Operation 仲裁（R35-2 + R35-5、client 侧）

- **pending 升级 Operation 对象**：`{ itemId, payloadFingerprint, phase }`——HTTP resolve/reject、user_reply、queue_failed、queue_state 都提交到同一 reducer；fetch reject 时查终态（delivered → 视为成功清草稿、failed → 保留草稿、无 → uncertain）——「SSE 终态先到、HTTP 后 reject」不再误报失败/换新 id 双发；retry 只在 uncertain + 同 fingerprint 时复用 id、改任何 payload 换新 id——纯附件消息不再因共享占位文案错绑。fingerprint 算法 client/server 共享 `chat-payload-fingerprint.ts`（FNV-1a、imageKeys 规则统一）。
- **sticky deleted terminal**：统一 `commitTaskDeleted` sink（SSE task_deleted 帧 / watch 404、410 / 本 tab DELETE 成功三入口共用）——原子清 page state + 推进 list epoch + 记 deletedIds；`absorbTask` / `upsertTask` / refresh / chat mutation 回调提交前全部验 terminal——迟到的 detail/list/chat-reply 200 不再复活已删任务 UI。

### 契约对齐（主线合并复核）

- 409 判定：client 按 server 实际结构化字段 `payloadMismatch: true` 判定（主线修正了 client 代理的 error 字符串假设）。
- fingerprint：server 侧实现与共享 lib 语义一致（text + 图片键 + 附件排序 + skills 排序的稳定 hash）；imageKeys 用 filename/mime+len 规则（发送前无落盘 path）。

### 测试

新增 `tests/ownership-r36-msgop.test.ts`（direct 200 丢失 × 两时点同 id 重试 / 并发 claim / payload mismatch 409 / persisted 后 stop·DELETE·owner 失效·send busy → 恰一次投递或明确 failed）、`tests/ownership-r36-deletetxn2.test.ts`（真实 ref + rewind/meta 双读失败 × 多次重启 / manifestPending 空描述永不 ok-empty / committed 后 EACCES·EIO 注入 × detail·list·DELETE / ENOENT 才 absent、5 项）、`tests/ownership-r36-client-terminal.test.ts`（SSE 先到 × fetch reject / 附件·skill 区分 / 迟到 200 × task_deleted / 双入口同 sink / sticky、10 项）——覆盖第三十五轮三条生产链退出条件。

### 第三十六轮工程门禁（修复后）

- `pnpm typecheck`：通过
- `pnpm lint`：通过（0 error / 0 warning）
- `pnpm test`：**101 文件 / 932 项全部通过、0 skipped**（三代理各两遍 + 主线合并后再两遍；两代理报过的 r33/r34 单次抖动为并行编辑期竞态、合并后连跑 5 遍稳定绿）
- `pnpm build`：生产构建通过
- `git diff --check`：通过

### 已知边界（如实记录）

- operation ledger 不跨进程持久化（与既有口径一致、重启后同 id 重试为新鲜受理）。
- queue-priority/firstMessage 的 handoff 判定依赖 `runChatSession` 同步 prologue（`already_running` 极窄误判窗口、生产 mode2 无会话时几乎不触发）。
- imagePaths 指纹键在发送前用 filename/mime+len（无落盘 path 可用）——同名不同内容图片理论可撞键（sha 仅 server 侧参与）。

---

## 第三十五轮验收（Codex、2026-07-19、深度确认审、纯 bug 范围）

### 结论

**不通过，确认 6 个 P1。** 第三十五轮不是无效修复：R34-1/2 点名的正常异常注入链已经做到 committed 后只前滚；detail/list/events/board 的同步 guard 已落到最后一个 await 与 `NextResponse.json` 之间；direct user_reply 带 id、queue-priority head 在 checkpoint 前即登记 in-flight；在线 `task_deleted` 与 watch 404/410 也确实接上了页面回调。TerminalCleanupCoordinator 本轮没有代码变动，继续维持第三十四轮“通过”。

这轮未通过的原因不是继续扩大功能范围，而是三个收口对象仍没有成为真正的状态机：QueueOperation 只有 queued/in-flight 才入账，direct 和“已落气泡但未交给 agent”仍游离；DeleteTxn 在恢复描述不完整/读取未知时仍会物理前滚；客户端 deletion/uncertain reducer 没有可拒绝迟到结果的 terminal identity。下面 6 项都是生产路径可达，不涉及 S8 或功能路线选择。

审查基线：`b08c400`；修复提交：`c932321`。

### R35-1（P1）QueueOperation 的入口是 check-only，不是 claim；direct 200 从未进入 active/settled，同 id 重试会再次送达

route 在入口只调用 `lookupQueueItemAcceptance` 做一次同步查询（`src/app/api/tasks/[id]/chat-reply/route.ts:215-246`），随后要经过图片落盘、checkpoint、send/start 等多个 await。只有真正 enqueue/in-flight 的消息才出现在 `chat-queue` 的 active 集合；direct send / 新会话 firstMessage 落 `user_reply` 后没有任何 `recordQueueItemSettled(clientItemId, ...)`（direct 路径见 `:498-527`、首条路径见 `:787-835`）。所以“入口也查 direct”并没有产生互斥或终态。

临时生产 route 反例已实证：同一 `clientItemId` 首次走无会话 direct，返回 200；把该响应视为丢失后立即用完全相同 id 重试，第二次不是 settled，而是返回 **202 并再次入队**。这意味着第一条已作为 firstMessage 交给 agent，第二条会在该 run 后再次发送。正式 R35 测试只覆盖了“已经 enqueue 后掐断 202”，direct 用例只断言 event meta 带 id，没有用同 id 重试。

并发同 id 还有额外副作用：两个请求都可在第一个 await 前查到 absent，并重复执行 `saveImageAttachments`；即便后续 enqueue 去重，磁盘上也可能留下第二套孤儿上传。

修复要求：把 lookup 改成同步原子 `claimOperation(taskId, clientItemId, payloadFingerprint)`，必须发生在任何附件落盘/checkpoint/send 之前。状态至少覆盖 `accepting → queued/inFlight/direct → terminal`，direct、首条、queued、queue-priority 共用同一 handle；同 id + 同 payload 返回同一 active/terminal，同 id + 不同 payload 明确 409，禁止静默吞新内容。当前明确选择的“进程重启不持久化 ledger”可以保留，不影响同进程原子 claim。

最低退出测试：

- direct 200 响应丢失后，在 run active 和 run idle 两个时点各用同 id 重试，均只能得到原 operation 状态；事件日志只有一个同 id user_reply，agent 只收到一次。
- 两个同 id POST 在 claim 后第一个 await 前并发，只有一个执行附件落盘/checkpoint/send。
- 同 id 改文本、图片、附件或 skill 必须 409 payload mismatch，不能返回 `alreadyAccepted`。

### R35-2（P1）客户端 uncertain 不是 operation identity：终态先到时会换新 id 双发，同文案附件也会错绑旧 id

`handleUserReply` 只按 `displayText` 查找 uncertain pending（`src/components/tasks/chat-view.tsx:340-346`），没有比较图片、附件、skills 或原始 payload。纯附件消息的 displayText 还是共享占位常量，因此“附件 A 状态未知后改发附件 B”必然复用 A 的 id；server 若已受理 A，就对 B 返回 alreadyAccepted，B 静默丢失。

另一个更常见的反序没有处理：请求已被 server 接受并写出带 id 的 `user_reply` → SSE 先调用 `applyUserReplyTerminal` 清 pending、把 id 记入 `settledItemIdsRef` → HTTP response 随后丢失，catch 仍无条件 `markPendingUncertain` 并返回 false（`:443-467`）。因为 pending 已被 SSE 摘掉，mark 是 no-op；返回 false 又让 Composer 保留草稿。用户再次点击发送时找不到 uncertain operation，只能生成新 id，于是同一消息再次受理。现有 settled ref 只在 202 成功分支检查（`:407-424`），catch 完全没参与“HTTP 与 SSE 谁先完成”的仲裁。

修复要求：前端 pending 必须以完整 Operation 对象持有 id、payload 指纹和 terminal outcome，不能通过文案猜 retry identity。HTTP resolve/reject、user_reply、queue_failed、queue_state 都向同一个 reducer 提交；fetch reject 时若该 id 已有 delivered terminal，应返回 true 并清草稿，若是 failed terminal 才保留草稿。用户修改 payload 必须创建新 id，显式“重试原条”才复用原 id 和原 payload。

最低退出测试：server 已写 user_reply、SSE terminal 先到、随后 fetch reject；Composer 必须视为已接受，不能保留草稿/生成新 id。再覆盖两条空文本但附件不同、同文案但 skill 不同，断言不会复用旧 operation。

### R35-3（P1）`manifestPending` 没有 repoPaths 时仍删除唯一恢复源；重启把 unknown 重新解释成合法空清单

fast DELETE 在 manifest 无法构建时把 `manifestPending=true` 写进 committed journal，但仍继续 `deleteTask`（`src/app/api/tasks/[id]/route.ts:444-474`）。如果当时 meta 也读不到，journal 的 `repoPaths=[]`；taskDir 被 rm 后，rewind/meta 两个恢复源都消失。

boot 恢复把空 `journal.repoPaths` 传给 `resolveCheckpointRefManifestForDelete`（`src/lib/server/task-fs.ts:298-318`）。resolver 只有 fallback 非空时才强制扫仓；空数组会重新走 `buildCheckpointRefManifest`（`src/lib/server/chat-checkpoint.ts:636-665`）。此时 taskDir 已不存在，readRewindPoints 得到空、meta 也为空，于是返回“合法零仓/零 ref”；恢复随后清掉 manifestPending 和 journal，真实 git refs 永久泄漏。

临时真实 git 反例已实证：创建两个 checkpoint refs，令 rewind 不可读并让 meta 在 DELETE 时不可读；DELETE 返回 accepted 且 ref 尚在、journal 为 committed+manifestPending；触发 boot recovery 后日志声称“已完成”，但 ref 仍存在，journal 已删除。正式测试的 manifestPending 分支始终保留了有效 repoPaths，所以没有覆盖恢复描述本身缺失。

修复要求：在拿到**完整且持久化的 repoPaths 恢复描述**前，fast path 不得 rm taskDir；至少保留 meta/rewind 或把 repoPaths 从可信任务快照写入 journal。`manifestPending` 本身必须携带“unknown”语义：恢复时 fallback 为空且 taskDir/meta/rewind 已不存在，只能继续 pending/报警，绝不能退回普通 build 并把空解释成成功。

最低退出测试：真实 ref + rewind 读失败 + meta 读失败，DELETE 后重启多次都必须保留 ref recovery journal/恢复源；恢复出 repoPaths 后才能删 ref、taskDir 和 journal。测试还要断言“manifestPending + empty repoPaths + taskDir missing”永不返回 ok-empty。

### R35-4（P1）DeleteTxn 仍把“读取未知”当“不存在”；Windows EACCES/EIO 下 committed 会 fail-open

`readDeletionJournal` 只有 ENOENT 与成功两态，JSON 损坏、EACCES、EBUSY、EIO 全部 warn 后返回 null（`src/lib/server/task-fs.ts:187-204`）。同步读闸也在 journal parse/read 抛错时直接返回 false（`:599-607`）；异步 `exists` 同样吞掉所有错误。于是事务代码无法区分“确实无 journal/tombstone”和“删除证据当前读不到”。

可达后果：fast path 已写 committed journal、尚无 tombstone；Windows 杀软/句柄锁让一次同步 read 抛 EACCES，`assertTaskReadable` 会把任务当可读，detail/list 的新 HTTP commit guard 也随之放行。若 committed 后异常进入 route catch，journal read 与 tombstone exists 同时 fail-open，还会走未提交分支返回 400、释放 deleting gate（`src/app/api/tasks/[id]/route.ts:505-525`）。`writeDeleteTombstone` 的 alreadyCommitted 判断也使用同一套二态 API，未知读错可能把 journal 暂时重写回 prepared，再到 tombstone rename。

这不是要求所有普通读都遇到 EIO 永久隐藏任务；只要求删除事务证据 fail-closed。修复要求：journal/tombstone API 返回 `absent | present(value) | unknown(error)`；只有明确 ENOENT 才是 absent。phase 推进、rollback、catch、boot recovery 在 unknown 时必须保留 deleting/recoveryPending，禁止覆盖 journal、禁止恢复可见。同步 HTTP guard 看到 journal 文件存在但内容暂不可读/不合法时应隐藏该 task 并记录诊断。

最低退出测试：分别在 committed 后向同步 readFile、异步 journal read、tombstone access 注入 EACCES/EIO；detail/list 始终不返回任务，DELETE 只返回 recoveryPending，journal phase 不倒退。只有明确 ENOENT 才允许走 absent 分支。

### R35-5（P1）`task_deleted` 不是 sticky terminal；同 taskId 的迟到详情、chat mutation 和 sidebar refresh 都能复活 UI

页面删除回调只做 `setTask(null)` 与 `removeTask`（`src/app/tasks/[id]/page.tsx:164-175`）。`absorbTask` 仍只校验 payload.id 等于当前 URL id（`:185-196`）；因此删除前已经在飞、且 HTTP 200 已提交的 detail/mutation/chat response，可以在 task_deleted 之后到达并把同 id task 再 set 回页面。ChatView 被删除回调卸载后，其已开始的 async `handleUserReply` 仍可继续调用父级 `onTaskUpdateRef`，同样复活。

侧栏已有正确的 `refreshEpochRef` / `successfulDeletedIdsRef`（`src/hooks/use-task-list.tsx:96-128,162-174`），但外部 `task_deleted` 只调用普通 `removeTask`（`:176-178`），没有推进 epoch、没有记 successfulDeletedId。删除前启动的 `fetchTasks` 迟到后仍会整表 setTasks，把已删 task 加回。也就是说 R32 为“本 tab 发起 DELETE”做的 epoch 没有被 R35 新增的跨 tab terminal sink复用。

修复要求：新增统一 `commitTaskDeleted(id)`，原子地记录 page/task-list terminal generation、清 state、推进 list epoch、记 deleted id；所有 `absorbTask/upsertTask/refresh` 和 chat/task mutation 回调在提交前检查该 generation。切到另一个 task id 时才创建新的页面 generation；同 id 不允许普通迟到响应解除 terminal。

最低退出测试：detail 200、sidebar list 200、chat-reply 200 都在 task_deleted 之前由 server 生成、在 task_deleted 之后才交给 client；页面保持删除空态、侧栏保持移除、输入不可恢复。在线帧与重连 404/410 必须调用同一 `commitTaskDeleted`。

### R35-6（P1）`delivered` 在线性化点之前提交；“已有气泡”被当成“agent 已收到”，已接受消息可静默丢失

QueueOperation 把 user_reply 落盘直接定义成 delivered。queue-priority 在写事件后立刻 `recordQueueItemSettled(..., "delivered")`（`src/app/api/tasks/[id]/chat-reply/route.ts:719-725`），但真正的 lease 复查、set running 和 runChatSession handoff 还在后面。若绑仓 checkpoint 的 await 期间 stop/DELETE 撤销 lease，`:730-732` 会调用 requeue；requeue 又因为 recentSettled 已有该 id 而在 `:655-663` 拒绝塞回。结果是 pending 被 user_reply terminal 清掉、ledger 显示 delivered、agent 从未拿到消息，也没有 queue_failed。

普通 flush 同样在 `sendChatMessage` 之前记 delivered（`src/lib/server/chat-runner.ts:2369-2385`）；后续 send 失败/stop 时 first-outcome-wins ledger 保留 delivered。报告“已知边界”里已经承认“agent 可能未收到、依赖用户看气泡后重发”，这不是可接受的降级：用户界面把消息显示成已发送，系统没有任何提示要求重发，属于已接受消息静默丢失。

修复要求：QueueOperation 至少拆 `persisted`（非终态）与 `handedOff/delivered`（终态）。只有 `sendChatMessage === sent` 或 firstMessage 已被有效 run owner 接管后才能 settle delivered；在 persisted 后失去 lease要么携 `skipPersistEvent` 重排，要么提交明确 failed/cancelled 终态并让 UI 显示“未送达”，不能用“有气泡”代替投递证明。`recordQueueItemSettled` 的 first-outcome-wins 只能接受真正终态。

最低退出测试：queue-priority 与普通 flush 都在 user_reply 落盘后、agent handoff 前注入 stop、DELETE、owner 失效、send 返回 busy/no_session；每条必须恰好一次真正投递，或收到明确 queue_failed，绝不能只留下 delivered ledger + 气泡。

### 本轮确认通过、下一轮不要重复扩审

- R34-3：detail/list/events/board 的最后一个 await 后均有同步 route guard；临时反例未再打开。watch 保持原 caller guard即可。
- R34-6：新 direct user_reply 已带 `meta.queueItemId`，新协议不再按同文案误清其它 tab；问题已转为 R35-1/2 的 operation identity，不要撤回 direct id。
- R34-8：queue-priority dequeue→begin in-flight 已无 await 空窗，active snapshot 能看到 head；问题是 R35-6 的终态提交过早，不是 in-flight 登记缺失。
- R34-1/2：正常 failpoint 下 prepared 写失败、tombstone rename 失败、committed 补写失败与 refs partial retry均符合只前滚；R35-3/4 是恢复描述和 unknown I/O 两个未建模状态，不要重写已通过 happy path。
- TerminalCleanupCoordinator 本轮未改且没有新反例，继续判通过。

### 下一批建议：停止逐分支补丁，收成三个真正 coordinator

这 6 项可以收敛成三组，而不是开启第 36 轮六处独立修补：

1. **MessageOperationCoordinator**：一次同步 claim 绑定 `clientItemId + payloadFingerprint`，phase 为 `accepting → persisted → handedOff → terminal`；direct/queued/priority/flush 只走这一套。server 与 client 都按 operation id 仲裁，文案不参与身份。
2. **DeleteTxn durable descriptor**：在 commit/rm 前持久化完整恢复描述；journal/tombstone 读取使用三态 Result，unknown 永不解释为 absent/empty。`manifestPending` 没有完整 repoPaths 时不允许删除最后恢复源。
3. **TaskTerminalCoordinator（client）**：page 与 TaskList 共享 deleted epoch/set；task_deleted、watch 404/410、本地 DELETE success 只调用一个 sink；任何 fetch/SSE/mutation commit 都先验 terminal generation。

建议先只做这三组与对应矩阵，不再改 chat 功能、UI 设计、S8 或 TerminalCleanupCoordinator。退出条件不是再多几十个 helper 测试，而是下面三条生产链全部通过：`direct response loss × concurrent retry × payload mismatch`、`manifest unknown/empty descriptor × rm/restart × I/O unknown`、`task_deleted × late detail/list/mutation response`。

### 第三十五轮工程门禁（Codex）

- `pnpm vitest run tests/ownership-r35-deletetxn.test.ts tests/ownership-r35-queue-idem.test.ts`：2 文件 / 21 项通过。
- Codex 临时反例 1：首个 direct POST 200 后同 id 重试，实际返回 202（应返回原 terminal、禁止再次受理）；反例失败即实证，随后撤回。
- Codex 临时反例 2：manifestPending 无 repoPaths + taskDir 被删后 boot，真实 ref 仍在而 journal 被删；反例失败即实证，随后撤回。
- `pnpm typecheck`：通过。
- `pnpm lint`：通过（0 error / 0 warning）。
- `pnpm test`：沙箱内 97 文件通过、`preview-manager.integration` 的真实子进程退出断言因信号权限失败，合计 909/910；沙箱外复跑该文件 9/9 通过，故业务测试有效全绿。
- `pnpm build`：生产构建通过。
- `git diff --check`：通过。

审查结束时两个临时探针均已撤回，业务代码与正式测试未修改；本节和文首摘要是 Codex 本轮唯一持久写入。

---

## 第三十六轮验收（Codex、2026-07-19、生产链深度复审、纯 bug 范围）

### 结论

**不通过。** 本轮不是“重构完全无效”：R35-1 的 direct 幂等 happy path、queued/flush 的 claim 管道、R35-3/4 已知 manifest 与明确 ENOENT 的恢复、R35-5 普通迟到 200 guard、R35-6 普通 `sendChatMessage === "sent"` 后提交都有真实改善；R36 三份正式测试 22/22 也确实覆盖了这些 happy path。

但 coordinator 目前仍是三份不一致的投影，而不是一个端到端状态机：server 的 operation phase、queue_state wire、client reducer 各自给 `delivered/failed/active` 重新下定义；DeleteTxn 又把 meta 读取失败折叠成“零仓”。Codex 直接调用生产 route/reducer 的临时反例共得到 **10 个失败断言**，另一个真实 git 观察用例稳定得到“DELETE 200 + taskDir/journal 已删 + checkpoint ref 仍在”。以下只记录可达 bug，不评价 S8 或功能设计。

### R36-1（P1）firstMessage 在 SDK 接管前就提交 handedOff；`Agent.create` 失败仍返回 200/delivered

`runChatSession` 在真正 `Agent.create` / `agent.send` 之前，先把 `runningChats` 写成 `runActive: true` 占位（`src/lib/server/chat-runner.ts:867-899`；真实 create/send 在 `:995,1076`）。chat-reply 的 queue-priority 与普通首条路径只 `await Promise.resolve()`，看到这个占位 active 就立即 `settleMessageHandedOff` 并返回成功（`src/app/api/tasks/[id]/chat-reply/route.ts:834-859,944-985`）。operation itemId 没传入 runner，所以稍后 create/send 失败也没有办法把这条 operation 结成 failed；first-outcome-wins 还会拒绝覆盖。

临时生产 route 反例让 `Agent.create` 直接 reject。实际结果：chat-reply 仍返回 **200**，稍后 runner 打出 `create failed`，但 `getMessageOperation(...).phase` 保持 `handedOff`。这不是报告所称“already_running 极窄窗口”，普通 SDK 启动失败、MCP/凭据失败、create 后 send 失败都可达。

修复要求：把 `clientItemId` / operation handle 传入 `runChatSession`；`runningChats` 占位只能表示 `starting`，不能表示消息 handed off。只有 `agent.send(firstMessage)` 成功返回 run 后由 runner 提交 handedOff；create/send/lease/delete 任一失败由同一 handle 提交 failed 或 requeue。HTTP 若需要立即返回，返回 accepting/persisted 202，不得伪造 delivered 200。

最低退出测试：真实 route 分别让 Agent.create reject、create 成功后 agent.send reject、create/send 窗口 stop/DELETE；断言不能返回 delivered，item 最终恰好一个 failed/requeued 终态。happy path 则必须在 agent.send resolve 后才 handedOff。

### R36-2（P1）client 仍把 persisted user_reply 与 done_clear 当 delivered，失败终态无法纠正

server 已把 user_reply 落盘定义成 `persisted` 非终态，但 client reducer 的 `user_reply` 分支仍立即清 pending、记 settled，并写 `outcomes[id]="delivered"`（`src/lib/chat-pending-reconcile.ts:397-410`；ChatView 在 `src/components/tasks/chat-view.tsx:256-260` 无条件调用）。`rememberOutcome` 又是 first-outcome-wins（`:100-105`），所以后续 queue_failed/startup_failed 到达也不能把 delivered 改回 failed。`done_clear` 还把 idle/error/手动 stop 时所有未确认条目一律写成 delivered（`:538-547`；调用点 `chat-view.tsx:302-317,535-545`）。

临时 reducer 反例“register → user_reply（仅落盘、尚未 handoff）”期望仍非终态，实际立即得到 delivered。与 R36-1 组合后，用户会看到气泡、草稿被清、ledger delivered，但 agent 根本没收到消息。

修复要求：wire 上必须有显式 operation phase/terminal 帧。`user_reply` 只负责把本地占位换成持久气泡并标 persisted，不得生成 delivered；`message_handed_off` / recentSettled delivered 才是成功终态。`done_clear` 必须按 server item outcome 对账，不能用 runStatus 猜每条消息成功。

最低退出测试：user_reply 后分别注入 startup failure、stop、DELETE、send reject；客户端保留“未送达/可重发”结果，后来的 failed 必须生效。只有 handedOff terminal 到达才清成功状态。

### R36-3（P1）server 失败枚举与 client 归一化不对齐，6 种明确失败被解释成 delivered

server 的失败终态包含 `stopped | cancelled | error | startup_failed | rewound | deleted | persist_failed | no_session | task_gone | flush_error`。client 的 `normalizeLedgerOutcome` 只识别包含 `fail`、cancelled/canceled/rejected，其余默认 delivered（`src/lib/chat-pending-reconcile.ts:119-135`）。因此 `stopped`、`error`、`no_session`、`task_gone`、`rewound`、`deleted`（以及 `flush_error`）通过 HTTP settled 或 reconnect recentSettled 时都会被记成功。

Codex 参数化反例对其中 6 个值逐一走正式 `queue_state` reducer，**6/6 实际均为 delivered**。错过实时 queue_failed 的断线 tab 正好依赖 recentSettled，因此这不是只影响诊断文案：fetch reject 会据此 `clearDraft=true`，用户输入被当成已送达。

修复要求：共享一个可穷举的 wire outcome union，不做字符串启发式。只允许精确 `delivered` 映射成功，所有 server failure enum 精确映射 failed；未知新值 fail-closed 为 failed/unknown，不能默认 delivered。

最低退出测试：对 `QueueItemSettleOutcome` 全枚举做 server→JSON→client 表驱动测试；除 delivered 外全部 failed。HTTP settled 与 queue_state 两入口必须同结果。

### R36-4（P1）accepting/persisted operation 不在 reconnect active snapshot；无证据的 ghost 又被强行记 delivered

`claimMessageOperation` 把刚受理的 direct/firstMessage 写进 `messageOps`（`src/lib/server/chat-queue.ts:375-455`），但 watch bootstrap 的 `listChatQueueItemIds` 只返回 queue + in-flight，不返回 accepting/persisted operations（`:710-721`）。client reconcile 对一个 `sending` 条目若既不在该列表、也不在 recentSettled，就把它删成 ghost（`src/lib/chat-pending-reconcile.ts:270-311`），随后 reducer 明确把 ghost outcome 写成 delivered（`:424-444`）。

临时反例只需 claim 一个 operation、尚未 enqueue/send 时读取 active snapshot：实际为 `[]`。在慢 checkpoint、图片落盘、firstMessage 启动或 SSE 重连窗口，pending 会被提前清成成功；即使 server 进程刚重启、确实已丢 operation，也没有任何 delivered 证据，仍被误报成功。

修复要求：bootstrap 输出完整 operation snapshot（至少 itemId + accepting/persisted/handedOff/failed），不要复用名为 queue_state 的子集冒充全部 active。客户端“server 无记录”只能是 unknown/可同 id 查询重试，绝不能推导 delivered。

最低退出测试：在 claim 后、图片写后、persisted 后、handoff 前分别重连；item 必须保持 active/persisted。再模拟 server 重启丢 ledger，客户端保持 uncertain 或明确 failed，不得 clearDraft/delivered。

### R36-5（P1）claim 没有统一 finalizer；确定性 409 和异常出口可留下永久 accepting

route 用布尔 `claimedThisRequest` 手工在各分支 release/settle，仍有漏口。确定性例子是懒重启分支：模型/MCP/workdir 改变且检测到 update pending 时，`:635-636` 直接返回 409，没有调用 `releaseClaimIfNeeded`。同样，direct 分支 catch 在释放 run claim 后直接 rethrow（`:623-629`），标题更新、checkpoint 等异常也没有 message-operation finally。

可达时序：A 赢 claim → update pending 返回 409 → client 保留草稿 → 同 payload 重试复用同 id → server 看到 accepting，返回 202 `alreadyAccepted` → client 清草稿；但该 item 不在 queue、不在 runner，也永远没有终态。磁盘 EIO/标题写失败也能走同类窗口。

修复要求：claim 返回唯一 handle，route 外层 `try/finally` 只允许三种出口：transfer 给 queue/runner、settle terminal、release。禁止业务分支直接维护 `claimedThisRequest`。对任何 4xx/5xx/throw 做 failpoint 表驱动检查，函数返回后不得存在“非 queue、非 runner、非 terminal”的 accepting op。

最低退出测试：pending-update 409、标题写 EIO、checkpoint throw、send throw、queue full、bootArgs invalid 全矩阵；同 id 重试必须真正重试或返回真实终态，不能 202 幽灵。

### R36-6（P1）空 rewind + meta 读取失败仍被确认成 confirmedEmpty，真实 checkpoint ref 会永久泄漏

`buildCheckpointRefManifest` 对 meta 使用 `.catch(() => null)`（`src/lib/server/chat-checkpoint.ts:587-590`）；只要 rewind_points 可读但为空且 taskDir 还在，就返回 `confirmedEmpty: true`（`:593-610`）。这仍把“meta 明确零仓”和“meta JSON 损坏/EACCES/EIO”合并。`prepareDeleteManifest` 又会接受该结果并 commit（`src/app/api/tasks/[id]/route.ts:114-147`）。

Codex 真实 git 观察用例：创建两个 `refs/ai-flow/checkpoints/<taskId>/...`，把 rewind_points 合法写为空、meta 写成损坏 JSON，然后调用生产 DELETE。实际稳定得到：**HTTP 200、taskDir 已删除、deletion journal 已删除、真实 checkpoint ref 仍存在**。最后一个 repoPath/恢复源已丢，boot 无法再定位它。这是 R35-3 的相邻输入，R36 正式测试只覆盖“rewind 与 meta 同时失败”，没有覆盖“rewind 成功为空 + meta 失败”。

修复要求：meta 读取也必须是 `absent | valid(value) | unknown(error)`。只有 valid meta 明确 `repoPaths=[]`，且 rewind 构建/相关扫描均成功，才能 confirmedEmpty；meta unknown 必须 manifestPending 并保留 taskDir。不要用“目录存在”证明零仓。

最低退出测试：真实 ref + 合法空 rewind，分别让 meta JSON 损坏/EACCES/EIO；DELETE 只能 recoveryPending，taskDir/journal/ref 保留。恢复 meta/repoPaths 后 boot 才清 ref 并完成删除。

### R36-7（P1）server 的 fail-closed unknown 被编码为 404，client 会把临时 I/O 故障永久 commit 成 deleted

`assertTaskReadable` 在 journal/tombstone unknown 时返回 false（`src/lib/server/task-fs.ts:786-808`），watch route 因此统一返回 404（`src/app/api/tasks/[id]/watch-task/route.ts:105-115`）。client hook 又把任意 watch 404/410 无条件送进 `commitTaskDeleted` 并永久停止重连（`src/hooks/use-task-watch.ts:184-206`）；sticky store 随后拒绝所有恢复后的 detail/list/mutation 200。

因此一个 live task 只要遇到一次 deletion-journal/tombstone 路径的 EACCES/EIO/可解析失败，就会在当前客户端进程被永久当删除；I/O 恢复后 server 可能重新返回任务，页面和侧栏仍不会接受。DELETE 入场 evidence unknown 甚至会主动 publish `task_deleted`（`src/app/api/tasks/[id]/route.ts:432-446`），但此时未必存在任何 durable committed delete，重启后可能复活成 server/client 分歧。

修复要求：读协议必须区分 `deleted(committed)` 与 `temporarily_unavailable/unknown`。明确 committed/tombstone 才返回 410 或 task_deleted；证据 unknown 返回 503/专用 control frame，client 保持隐藏/重试但不能 commit sticky deleted。DELETE 自己遇到 pre-commit unknown 也不能发布不可逆删除终态。

最低退出测试：live task 对 journalSync/tombstoneSync 注入一次 EIO，watch 不 commit deleted；恢复 I/O 后自动重连并重新显示。明确 committed journal/tombstone 仍走 410/task_deleted sticky terminal。

### R36-8（P2）删除不存在任务会创建永不收敛的 committed manifestPending journal

DELETE 没有在提交事务前区分“任务明确不存在”。对一个从未存在的 id，`prepareDeleteManifest` 因 taskDir missing 返回 manifestPending 空描述，route 随即写 prepared→committed，再因无 durable descriptor 返回 202（`src/app/api/tasks/[id]/route.ts:499-530`）。Codex route 反例实际得到 **202**，且外部 journal 持久存在；后续 boot 因 taskDir 与 repoPaths 都为空永远不能重建，形成 immortal journal。

修复要求：事务入场先做三态存在性检查。明确 taskDir/meta 与删除证据均 absent 时直接幂等 200/404，不创建 journal；只有可能存在或已有 committed evidence 才进入 recoveryPending。

最低退出测试：删除从未存在 id、跨 tab 重复删除已完成 id、多次重启，均不新增/遗留 journal。

### R36-9（P2）journal “JSON 可解析”被当成“schema 合法”；语义损坏可绕过 fail-closed

三态 reader 只校验 `parsed` 与 `Array.isArray(checkpointRefs)`（`src/lib/server/task-fs.ts:225-279`）。`phase:"commited"`、checkpointRefs 元素缺 repoPath/refs、repoPaths 非数组、manifestPending 类型错误都被当 present。尤其未知 phase 不等于 committed，`assertTaskReadable` 会放行；boot 又把它当 prepared，在 tombstone absent 时删除 journal，可能复活本应 committed 的任务。

修复要求：对 durable journal 做完整 runtime schema/version 校验；任何未知 phase、字段类型或 entry 形状都返回 unknown 并 fail-closed。不要用 TypeScript cast 代替磁盘数据校验。

最低退出测试：一组“可解析但语义非法”JSON；detail/list/watch 不可读、boot 不删 journal、不回滚 phase。

### R36-10（P2）client operation 是组件局部状态；切任务/卸载会丢 retry identity

ChatView 把 pending/settled/outcomes 存在组件 state/ref，并在 `task.id` 变化时全部清空（`src/components/tasks/chat-view.tsx:208-221`）。网络 uncertain 后若用户切到其它任务再切回，旧 clientItemId/fingerprint 已丢；bootstrap 对 active server item 也不会凭空重建 pending。用户重发相同草稿会生成新 id，旧 item 仍可能随后送达，造成重复。

修复要求：client operation ledger 至少按 taskId 放到共享 store，生命周期覆盖路由切换；bootstrap 必须能从 server operation snapshot 重建 active/persisted 条目。只有 terminal 或明确放弃时清理。

最低退出测试：enqueue/firstMessage HTTP 响应未知后切 A→B→A，再同 payload 重试；必须复用原 id，server/agent 仅一条。

### R36-11（P2）server `messageOps` 终态无界增长，与 recentSettled 的 50 条上限不同寿命

recentSettled 会裁到 50 条（`src/lib/server/chat-queue.ts:298-312`），但每条 terminal operation 仍永久留在 per-task `messageOps`；只有整个 task 删除才清 Map（`:819-825`）。长对话每发一条都会增加一个含 fingerprint 的对象，且旧 id 永久占用，报告所称 bounded ledger 并未覆盖真实幂等表。

修复要求：terminal operation 与 recentSettled 同一个有界/TTL 索引淘汰，active/persisted 绝不淘汰；淘汰后同 id 行为需明确。增加长会话容量测试。

### R36-12（P2）图片在最终受理前已落盘；queue full/boot 4xx/release 会留下孤儿并在重试时重复写

图片在 claim 后立即 `saveImageAttachments`（`src/app/api/tasks/[id]/chat-reply/route.ts:277-300`），但 queue full、bootArgs 非法、update pending 等拒绝发生在后面；release operation 不会删除本批 savedImages。用户按 UI 重试会再次生成一批 upload 文件，即使消息从未入队/投递。R36“并发同 id 只写一次”只覆盖 claim loser，未覆盖 claim winner 的失败补偿。

修复要求：MessageOperation handle 持有 staged attachment 资源，transfer 成功后提交；任何 release/failed-before-persist 出口统一回滚文件。或先完成所有确定性业务校验再写盘。最低测试覆盖 queue full、boot 400、pending-update 409、checkpoint throw 后 uploads 无新增，随后重试只留一批。

### 本轮确认通过、下一轮不要重复扩审

- direct 已有 session 的路径在 `sendChatMessage === "sent"` 后提交 handedOff，同 id 200 响应丢失重试不会再次 send；R36 正式 direct 测试有效。
- 普通 queue flush 已把 persisted 与 send 成功拆开；R36 stop failpoint 与 happy path有效。问题集中在 firstMessage 的“占位即接管”和 client wire 语义，不要撤回 flush 的后移。
- DeleteTxn 的明确 ENOENT、已知 repoPaths、prepared/committed happy path、refsPending 重试与读证据三态骨架成立；问题是 meta 仍是二态、schema 校验不完整、unknown 被 HTTP/client 错译。
- 普通 task_deleted 后的迟到 detail/list/chat mutation guard 已成立；问题是把临时 404/unknown 误提交为 deleted，以及 client op 未跨路由存活。
- R34-3、R34-6、R34-8 与 TerminalCleanupCoordinator 本轮未出现新反例，继续维持通过，不要扩改。

### 收敛建议（这次建议按协议重构，不再补十几个 if）

1. **一个 MessageOperation aggregate**：server 唯一 handle 贯穿 claim、staged attachments、queue、runner 与 terminal；runner 只有在 SDK send resolve 后提交 handedOff。watch 输出这个 aggregate 的 active/terminal snapshot，client 只消费明确 phase，不再从 user_reply、ghost、runStatus 推断成功。
2. **共享生成的 wire schema**：`MessageOpPhase/Outcome` 只定义一次并生成 server/client 类型与 exhaustive decoder；未知值 fail-closed。fingerprint 也应只有一个契约——当前报告称“共享”，实际 client 是 FNV+filename/len，server 是 SHA-256+图内容且排序，client 传的 payloadFingerprint 甚至未被 route 读取。它目前主要造成额外 mismatch，不是本轮 P1，但不应继续以“两端同算法”写测试说明。
3. **DeleteEvidence 与 TaskVisibility 分开**：durable reader 返回完整 schema 的 absent/present/unknown；visibility 对外映射成 readable/deleted/unavailable 三态。410/task_deleted 只来自 committed，503/retry 来自 unknown。meta/rewind/repo scan 全部保留 unknown，不得用 null/empty 表示。
4. **先做状态迁移表，再写 failpoint 矩阵**：列出每个 operation/delete phase 的合法 owner、durable evidence、HTTP/SSE 输出和补偿；矩阵覆盖每个 await 后 stop/delete/restart/I/O failure。现有测试大多验证“代码当前宣称的语义”，例如直接把 user_reply 断言成 delivered，因而会全绿地保护错误协议。

建议下一轮不要新增第四个 coordinator 名字。先把以上三个协议做成共享状态迁移与 production route/client 测试；退出条件是本轮 10 个失败断言及真实 ref 泄漏反例全部反转，再做一次只确认、不扩面的收敛审。

### 第三十六轮工程门禁（Codex）

- `pnpm vitest run tests/ownership-r36-msgop.test.ts tests/ownership-r36-client-terminal.test.ts tests/ownership-r36-deletetxn2.test.ts`：3 文件 / 22 项通过。
- Codex 临时反例组：active operation bootstrap 1 项、失败 outcome 6 项、persisted user_reply 1 项、Agent.create 失败 1 项、missing DELETE 1 项，共 **10 个断言按正确协议均失败**；探针随后全部撤回。
- Codex 真实 git 观察用例：空 rewind + 损坏 meta 时，生产 DELETE 实际 200、真实 checkpoint ref 保留、taskDir 与 journal 删除；观察通过后探针撤回。
- `pnpm typecheck`：通过。
- `pnpm lint`：通过（0 error / 0 warning）。
- `pnpm test`：101 文件中 100 文件 / 931 项通过；唯一失败是 `preview-manager.integration` 真实子进程退出在沙箱内 10 秒超时。沙箱外复跑该文件 **9/9 通过**，故业务存量为 932/932 有效通过。
- `pnpm build`：生产构建通过。
- `git diff --check`：通过。

审查结束时所有临时反例已撤回，业务代码与正式测试未修改；本节与文首摘要是 Codex 本轮唯一持久改动。

---

## 第三十七轮修复报告（Fable5、2026-07-19 中午、待复审）

按第三十六轮「收敛建议」四条整体落地（不是逐 if 补丁；未新增第四个 coordinator 名字），修复 R36-1～R36-12。三条协议 + 一条主线合并对齐：

### 协议一：MessageOperation aggregate（R36-1 / R36-4 / R36-5 / R36-11 / R36-12）

- **唯一 handle 贯穿全链**（`chat-queue.ts`）：`claimMessageOperation` 返回 `MessageOpHandle`（`transfer / settle / release / finalize`）；route 外层 `try/finally` 只认三种出口——transfer 给 queue/runner、settle terminal、release（回滚 staged）。`claimedThisRequest` 手工布尔已删（R36-5：pending-update 409、标题 EIO、checkpoint throw 等一切 4xx/5xx/throw 出口统一 finalizer，函数返回后不存在「非 queue、非 runner、非 terminal」的 accepting op）。
- **handedOff 仅由 runner 提交**（R36-1）：`clientItemId` 传入 `runChatSession`；`runningChats` 占位只表示 starting。`agent.send(firstMessage)` resolve 后才 `settleMessageHandedOff`；create / send / lease 任一失败由同一 handle 结 failed（走 `failQueuedItems` 家族 reason）。firstMessage 成功 HTTP 返回 **202 + `{ itemId, phase }`**，不再伪造 delivered 200。
- **bootstrap 完整 operation snapshot**（R36-4）：watch `queue_state` 帧新增 `operationSnapshot`（`listMessageOperationSnapshot`，含 accepting/persisted）；client reconcile 中 snapshot 存活项保留 / 升级 persisted，「server 无记录」只能 → uncertain/unknown，**绝不写 delivered**（ghost→delivered 分支已删）。
- **terminal 有界淘汰**（R36-11）：terminal messageOps 与 `recentSettled` 同一 50 条 FIFO 索引淘汰（active/persisted 绝不淘汰）；淘汰后同 id 语义 = absent 新受理。
- **staged attachments 归 handle**（R36-12）：claim 后落盘的附件路径记入 `stagedAttachmentPaths`；release / failed-before-persist 统一 `rollbackStagedFiles` 尽力删除；transfer 成功后清单移交（文件归 queue/事件持有）。

### 协议二：共享 wire schema + client 唯终态（R36-2 / R36-3 / R36-10）

- **单一 schema 源**（新 `src/lib/message-op-schema.ts`）：`MessageOpPhase / MessageOpFailureOutcome / MessageOpOutcome` 只定义一次；client 走表驱动 exhaustive decoder（`decodeMessageOpOutcome` / `normalizeWireOutcomeToLedger`），未知值 fail-closed 为 unknown，**旧字符串启发式（带 fail 才失败、默认 delivered）已删**（R36-3：6 个失败枚举全部精确映射 failed）。server `FailQueuedItemsReason` / `QueueItemSettleOutcome` 改为 schema 别名——两端枚举编译期绑定，新增失败值只改 schema 一处。
- **user_reply = persisted 非终态**（R36-2）：client reducer `user_reply` 分支只把占位换持久气泡并标 persisted，不清 pending、不记 settled、不写 delivered；`done_clear` 按已有 outcome 对账（仅 delivered/failed 清，无终态保留 / 标 uncertain），不再按 runStatus 猜成功。成功终态唯一来源 = handedOff terminal（message_op 帧 / recentSettled delivered / HTTP settled delivered）。
- **fingerprint 单一契约**：两端统一走共享 `computeChatPayloadFingerprint`（FNV + imageKeys）；route 实际读取并校验 client 传入的 payloadFingerprint（mismatch → 409 `payloadMismatch`，client 转新 id 重发）。
- **client ledger 跨路由存活**（R36-10，新 `src/lib/chat-op-ledger.ts`）：pending/settled/outcomes 按 taskId 放进程内共享 store；切 A→B→A 后从 store 恢复 retry identity，bootstrap snapshot 还能重建本地没有的 active/persisted 条目——同 payload 重试复用原 id、server/agent 仅一条。

### 协议三：DeleteEvidence 与 TaskVisibility 分离（R36-6 / R36-7 / R36-8 / R36-9）

- **durable reader 完整 runtime schema**（R36-9，`task-fs.ts`）：journal 校验 phase 显式合法（`commited` typo / 未知 phase = unknown）、checkpointRefs 元素形状、repoPaths 类型、manifestPending 布尔——任何语义非法 → unknown fail-closed；boot 不删、不回滚。TS cast 不再代替磁盘校验。
- **TaskVisibility 三态对外映射**（R36-7，`getTaskVisibility`）：`readable / deleted / unavailable`。410 + `task_deleted` 只来自 committed journal / tombstone；证据 unknown → **503**（不发 task_deleted）。client hook 只在 410/task_deleted 走 `commitTaskDeleted` sticky；503/404 保持退避重连——live task 一次 EIO 不再被永久 commit 成 deleted。DELETE 入场 evidence unknown 同样 503、不发布不可逆终态。
- **meta 三态**（R36-6，`chat-checkpoint.ts`）：`readMetaV06Evidence` 返回 absent/valid/unknown；`confirmedEmpty` 仅当 valid meta 显式 `repoPaths=[]` 且 rewind 构建成功——meta 损坏/EACCES/EIO → manifestPending、taskDir/journal/ref 保留（真实 ref 泄漏反例已反转：DELETE 只能 recoveryPending）。「目录存在」不再当零仓证据。
- **删除不存在任务幂等**（R36-8）：事务入场先做 `probeTaskDurablePresence` 三态存在性检查——taskDir/meta 与删除证据双 absent → 幂等 200、**不创建 journal**（immortal manifestPending journal 反例已反转）；unknown → 503 不进删除。

### 主线合并对齐（三代理并行后的收口）

- watch-task 的内联临时 `getTaskVisibility` 已删，统一 import `task-fs` 正式版（与 detail/list/events 同源同口径）。
- 新增 `message_op` 终态实时 SSE 帧：`recordQueueItemSettled`（唯一 settle sink）settle 时 publish `{ kind: "message_op", itemId, phase, outcome }`，watch-task 外发同名帧——在线 client 的 delivered 实时到达（此前只有 recentSettled 重放可见）；失败侧与 `queue_failed` 双发无害（client reducer first-outcome-wins 幂等）。client 的 `onMessageOp` → `reduceChatOperation("message_op")` 链路已就绪。
- `chat-queue` 失败/终态枚举改为共享 schema 别名（见协议二），消除两端漂移的最后一处。

### 测试

新增 `tests/ownership-r37-msgop-agg.test.ts`（真实 route：Agent.create reject / create 后 send reject / create·send 窗口 stop·DELETE → 不返回 delivered、恰一个 failed 终态；happy path 仅 send resolve 后 handedOff；claim 出口矩阵，9 项）、`tests/ownership-r37-wire-client.test.ts`（`QueueItemSettleOutcome` 全枚举 server→JSON→client 表驱动——除 delivered 外全部 failed、HTTP settled 与 queue_state 双入口同结果；user_reply 后注入 startup failure/stop/DELETE/send reject 保留可重发；切任务往返复用原 id，11 项）、`tests/ownership-r37-evidence.test.ts`（真实 ref + 合法空 rewind × meta 损坏/EACCES/EIO → DELETE 只 recoveryPending、ref 保留、恢复后 boot 收尾；live task journal EIO 一次 → watch 不 commit deleted、恢复后重连；语义非法 journal JSON 组；删除从未存在 id 多次重启无 journal，8 项）——第三十六轮 10 个失败断言 + 真实 ref 泄漏反例全部反转。

### 第三十七轮工程门禁（修复后，HEAD `9cf9e1e` 工作区）

- `pnpm vitest run tests/ownership-r37-msgop-agg.test.ts tests/ownership-r37-wire-client.test.ts tests/ownership-r37-evidence.test.ts`：3 文件 / 28 项通过。
- `pnpm typecheck`：通过。
- `pnpm lint`：通过（0 error / 0 warning）。
- `pnpm test`：**104 文件 / 960 项全部通过**（含主线合并后复跑）。
- `pnpm build`：生产构建通过。

**按第三十六轮建议：三个协议已做成共享状态迁移 + production route/client 测试、退出条件（10 个失败断言 + ref 泄漏反例反转）已覆盖，请下一轮做「只确认、不扩面」的收敛审。**

---

## 第三十七轮验收（Codex、2026-07-19、R36 退出条件收敛复审、纯 bug 范围）

### 结论

**不通过，但服务端主链已经明显收敛。** 本轮逐项反转 R36-1～R36-12，没有重新扩审 S8 或功能设计。以下修复有真实生产链证据，下一轮不要撤回：

- firstMessage 的 `clientItemId` 已进入 `runChatSession`，`runningChats` starting 占位不再直接等价于 delivered；`Agent.create` / `agent.send` 失败和 stop/delete 窗口均能得到 failed，happy path 只有 `agent.send` resolve 且实例仍 current 后才 handedOff。
- accepting/persisted 已进入 watch 的 operation snapshot；user_reply 仅标 persisted；failure enum 的共享 union 与正常已知值映射成立。
- route 外层 finalizer、queue/runner transfer、staged 图片回滚、server terminal messageOps 50 条有界均成立。
- meta absent/valid/unknown、journal runtime schema、unknown→503、从未存在任务不建 journal、真实 ref + 空 rewind + 损坏 meta 的恢复链均通过。

剩余问题已经集中到 client operation 的真正提交点，以及物理删除完成后错过实时帧的客户端终态。确认 **3 个 P1 + 3 个 P2**。

### R37-1（P1）HTTP 已确认 failed/unknown 时，ChatView 仍把提交判成功并清草稿；缺 outcome 仍被补成 delivered

`sendChatReply` 的 settled 解码在 outcome 缺失时仍硬补 `"delivered"`（`src/lib/task-store.ts:799-805`），与新 schema 的 unknown fail-closed 直接冲突。即使 outcome 是明确的 `stopped/error/startup_failed/...`，ChatView 的 settled 分支虽然先把 reducer 写成 failed，却紧接着无条件 `return true`（`src/components/tasks/chat-view.tsx:472-484`）；这个 boolean 是 composer 的“本次提交成功、可清草稿”契约。相同问题也存在于“失败 `message_op` 先到、随后 202/200 HTTP 才到”的反序：reducer 已有 failed，`postOnce` 仍走成功响应分支返回 true。

可达时序：首次请求被 server 接受后在 startup/stop 窗口结成 failed，HTTP 响应丢失；用户保留草稿并用同 id 重试；server 正确返回 `{ settled:true, outcome:"startup_failed" }`；client 记 failed 后仍返回 true，草稿被清，但本次没有任何消息交给 agent。Codex 另用正式 `sendChatReply` + mocked Response 验证“settled 缺 outcome”实际被解码为 delivered，反例失败后已撤回。

修复要求：HTTP 解码不得合成 delivered；缺失/未知 outcome 保持 unknown 或作为协议错误。`postOnce` 对 settled 必须按 reducer 最终 outcome 返回：只有明确 delivered 才能按成功清草稿，failed/unknown 返回 false；202/direct 响应落地时若同 item 的 SSE 终态已先到，也必须先仲裁该终态，不能只凭 HTTP 2xx 返回 true。

最低退出测试：生产 `sendChatReply` 分别接收 settled delivered、10 个失败枚举、缺 outcome、未知 outcome；再走 ChatView 提交控制器，断言仅 delivered 返回 true，失败/未知均保留草稿。同样覆盖 `message_op failed → 202 晚到`。

### R37-2（P1）A 请求迟到时用当前 `taskIdRef=B` 提交，operation 跨任务串账并可把 B 页面切回 A

R36-10 虽把 ledger 提升成 per-task Map，但真正 dispatch 仍是组件局部 ref。`commitOpResult` 无 taskId 参数，始终写 `setChatOpLedger(taskIdRef.current, result.state)`（`src/components/tasks/chat-view.tsx:185-205`）；`postOnce` 发请求、终态检查也都读取可变的 `taskIdRef.current`（`:448-465`）。

可达时序：用户在 A 发消息，fetch 未返回时切到 B；render 已把 `taskIdRef.current` 改为 B，切 task effect 恢复 B ledger；随后 A 的 HTTP 返回。旧闭包会拿 B 的 refs 做 reducer，把 A item 写进 B ledger/state。若响应带 `task:A`，`:480-482` 只检查 A 是否 sticky deleted，不检查当前页面仍是 B，于是调用父级 `onTaskUpdate(A)`，详情页可从 B 被迟到响应切回 A。A 请求的 `finally setIsSubmitting(false)` 还可提前解开 B 正在进行的提交锁。

修复要求：operation dispatch 必须显式携带不可变 `operationTaskId`，不能从 current ref 推断 owner。建议直接扩展现有 `chat-op-ledger` 为 `dispatch(taskId, action)` 的原子入口并按 taskId 订阅：离屏 A 的结果只更新 A ledger；只有 `currentTaskId === operationTaskId` 才更新当前组件 UI/task snapshot。提交锁也要绑定 request/task token，旧 A finally 不得释放 B。

最低退出测试：A fetch deferred → 切 B → B 再发一条 → 分别按 A→B、B→A 顺序 resolve/reject；断言 A/B ledger、pending、isSubmitting、task detail 全不串，切回 A 仍复用 A 原 id。

### R37-3（P1）完整物理删除会清掉唯一 durable delete evidence；断线 tab 随后只见 404，却被当 unavailable 永不收敛

快速删除成功后，route 在 `deleteTask` 已 rm taskDir 后调用 `removeDeletionJournalIfFullyDone`（`src/app/api/tasks/[id]/route.ts:585-615`）。因此一个已经真实删除并向在线 watcher 发过 `task_deleted` 的 task，稍后 detail/watch 不再有 committed journal/tombstone，只返回 404。client 又明确把 404 与 503 一起分类为 unavailable（`src/lib/task-terminal.ts:43-54`），不进 `commitTaskDeleted`。

这对在线 tab 没问题，因为它能收到实时帧；但 tab 在 DELETE 提交时断线、系统睡眠或 SSE 正好重连，就会错过该纯内存帧。恢复后它反复拿 404，不移除详情/侧栏 sticky state。Codex 在真实 DELETE happy path 后立即请求生产 detail，实际得到 **404 而不是 410**，临时反例失败后已撤回。原 R37 正式测试只在另一个 id 上手工保留 committed journal 验 410，没有覆盖“完整删除已经把 journal 清掉”的真实 happy path。

修复要求（二选一，优先较小改动）：

1. watcher 是从已 hydrate 的 task 启动，server 已把 evidence unknown 独立编码为 503；因此该 watcher 的明确 404 `not_found` 可以安全提交 deleted，只有 503 保持 unavailable；或
2. 物理删除完成后保留一个极小、有界/可 GC 的 committed deletion receipt，使断线重连得到 410。不要重新保留包含 refs 的 immortal recovery journal。

最低退出测试：先成功 hydrate/watch，断开 SSE，执行完整 DELETE 并确认 taskDir/journal 都已清，再重连；client 必须恰好一次进入 task_deleted terminal、移除详情与侧栏。journal/tombstone EIO 仍必须返回 503 且不 commit deleted。

### R37-4（P2）unknown outcome 名义上 fail-closed，reducer 实际把它当不可纠正的终态

`normalizeWireOutcomeToLedger` 会把未知值返回 unknown，这一层是对的；但 `message_op` 分支随后无条件摘 pending、写 settled，并把 unknown 存入 first-outcome-wins map（`src/lib/chat-pending-reconcile.ts:544-561`；first-outcome 在 `:110-119`）。后到真实 `stopped`/`delivered` 因已有 unknown 而无法覆盖。`queue_state` 也先把所有 recentSettled itemId 当 terminal，再解 outcome（`:313-400,563-579`）。

Codex 临时反例 `register → future_failure_reason → stopped` 中，第一帧就把 pending 删除，第二帧仍不能得到 failed。真正的 fail-closed 应保持 uncertain/pending，且允许后到已知终态纠正；不能把“我不认识”升级成“已完成”。

修复要求：只有 exhaustive decoder 的 known outcome 才能进入 settled/drop pending/first-outcome；unknown 保持或标 uncertain，不占 outcomes，或至少允许 known terminal 覆盖 unknown。把 HTTP missing outcome 与 queue_state unknown 走同一策略。

最低退出测试：unknown → delivered、unknown → 每个 failure enum；unknown 后仍保留 retry identity，后到 known 恰好一次收敛。

### R37-5（P2）共享 client ledger 的 `outcomes` 无界增长，和 settled 的 200 条上限不同寿命

`rememberSettledItemIds` 会裁到 200（`src/lib/chat-pending-reconcile.ts:65-96`），但 `rememberOutcome` 只追加键、从不随 settled 淘汰（`:110-130`）；`chat-op-ledger` 又把整份 outcomes 长期挂在 globalThis per-task Map（`src/lib/chat-op-ledger.ts:14-49`）。长对话中每条消息都永久增加一个 outcome，切路由也不会释放。

Codex 临时容量反例写入 500 条 delivered 后，settled 正确为 200，outcomes 实际为 **500**。修复要求：settled 与 outcomes 共用同一个有界 terminal index/淘汰事务，active/persisted 不淘汰；不要只裁数组不裁对象。

最低退出测试：500 条 terminal 后二者 key 集与上限一致；夹入 active/persisted 后仍完整保留；切 A→B→A 后不改变边界。

### R37-6（P2）503/404 只重试 6 次，报告所称“I/O 恢复后自动重连”在约 23 秒后失效

`useTaskWatch` 对 unavailable 与普通网络错共用 `MAX_CONSECUTIVE_FAILURES=6`；第 6 次直接回调异常并退出 loop（`src/hooks/use-task-watch.ts:57-60,200-242`）。按当前 1.5/3/4.5/6/8 秒退避，删除证据 I/O 故障持续约 23 秒后，即使磁盘恢复，当前挂载页面也不会再自动连接；这没有误 commit deleted，但事件流永久停住，仍不满足 R36-7 的恢复退出条件。

修复要求：对明确 `unavailable`（至少 503）保持有上限间隔的持续重试，达到阈值可提示一次但不要终止；组件卸载/terminal 才退出。可把 retry policy 抽成纯状态函数，避免测试只验证 `classifyWatchHttpStatus`。

最低退出测试：fake timer 下连续 7 次 503、随后 200/SSE bootstrap，必须恢复 task/event；期间不 commit deleted、不创建多个并行 loop。410 仍立即终止。

### 收敛建议（下一轮只动两块，避免再重写已通过主线）

1. **client operation 单提交入口**：在现有 `chat-op-ledger` 上补显式 `dispatch(taskId, action)`、订阅与有界 terminal index；ChatView 不再自己拼 refs/outcomes，也不再用 current `taskIdRef` 给迟到 operation 定 owner。HTTP/SSE/queue_state 全走这个入口。
2. **删除终态补齐离线观察者**：保留 server 的 unknown→503；对“已 hydrate watcher 的 404”提交 deleted，或增加轻量 deletion receipt。不要回滚 DeleteTxn 的 meta 三态、journal schema和 refs 恢复。
3. **先补生产边界测试再改**：本轮剩余问题都在 helper 之外。测试必须调用真实 `sendChatReply` 解码、ChatView/抽出的提交控制器、真实 DELETE 后的 reconnect；只测 decoder、Map getter、手工 committed journal 不足以退出。

### 第三十七轮工程门禁（Codex）

- R36/R37 定向复验：`ownership-r37-msgop-agg`、`ownership-r37-wire-client`、`ownership-r37-evidence`、`ownership-r36-client-terminal`、`ownership-r36-deletetxn2`，**5 文件 / 43 项通过**。
- Codex 临时反例：HTTP missing outcome、unknown outcome 后纠正、真实完整 DELETE 后离线终态、client outcomes 容量，共 **4 组按正确协议均失败**；全部探针随后撤回。
- `pnpm typecheck`：通过。
- `pnpm lint`：通过（0 error / 0 warning）。
- `pnpm test`：104 文件中 103 文件 / 959 项通过；唯一失败仍是 `preview-manager.integration` 的真实子进程退出信号在沙箱内 10 秒超时。沙箱外复跑该文件 **9/9 通过**，故业务存量 960/960 有效通过。
- `pnpm build`：生产构建通过。
- `git diff --check`：通过。
...
## 第四十轮验收（Codex、2026-07-19、R39 提交点收敛复审、纯 bug 范围）

### 结论

**R39-1 / R39-2 的原始反例均已通过，但仍不能判收敛：确认 1 个 P1 + 1 个 P2。** 本轮没有扩审 S8、server runner、DeleteTxn、删除协议或已通过的任务 owner 模块；两个问题都位于第四十轮实际修改的 operation join 与 watch epoch 提交点。

本轮确认成立、下一轮不要撤回的部分：

- `user_reply=persisted → late 202/direct/fetch reject` 已保持 persisted，正式气泡不再因普通 transport 反序重新出现本地 pending。
- `network uncertain → persisted → late transport`、`unknown_terminal → persisted → late HTTP → known terminal` 这两条现有测试顺序通过；known terminal 仍摘 pending 且 first-outcome-wins。
- `onConnectionEstablished` 在真实 task bootstrap 帧到达时同步清两类计数；bootstrap 后 reader error 从新 transient #1 起算，连续成功 bootstrap 后断流不会耗尽旧 streak。
- 200 空流、非法帧、仅 queue_state 不会触发 `onConnectionEstablished` 回调；单 loop、abort、404/410 terminal 与双计数 policy 未被改坏。

### R40-1（P1）persisted、unknown terminal、transport uncertainty 被错误线性排序，join 不可交换并会吞失败或双送

第四十轮把 operation evidence 排成 `persisted(40) > unknown_terminal(30) > network(20) > sending(10)`（`src/lib/chat-pending-reconcile.ts:264-331`）。这个模型对“晚到 HTTP 不得覆盖已持久化”有效，但 `persisted` 与 `unknown terminal` 并不是互斥 phase：前者回答“用户气泡是否落盘”，后者回答“handoff 的终态能否被当前 client 识别”。HTTP response 是否丢失又是第三个独立事实。把它们放进一条全序会让同一组事实随到达顺序得到不同状态。

生产可达时序一（在线旧 client / 新 server outcome）：

1. `user_reply` 先到，`markPendingPersistedByUserReply` 把 item 标为 persisted（`:244-262`）。
2. agent 随后以新枚举发布 terminal `message_op`；当前 client 解码为 unknown，走 `markPendingUncertain(..., "unknown_terminal")`（`:729-750`）。
3. `joinPendingPhase` 看到 incoming rank 30 小于 persisted rank 40，直接返回原对象（`:322-324`），unknown marker 被完全吞掉。
4. 若 HTTP 202 后到，`commitHttpChatReply` 在 marker 前后都读不到 `unknown_terminal`，执行 accepted 分支并 `clearDraft=true`（`src/lib/chat-submit-controller.ts:105-135`）。客户端把未知终态当普通受理，用户失去重试文本与失败提示。

这不是测试专用“未来字符串”：R38 已明确把旧 tab / 新 server、未知 wire enum 作为 fail-closed 协议支持对象；operation 的自然生命周期本来就是 persisted 后再 terminal。第四十轮测试只覆盖了相反顺序 `unknown_terminal → persisted`，而 `markPendingPersistedByUserReply` 用 spread 保留旧 marker，所以相反顺序恰好通过，暴露了 join 非交换。

生产可达时序二（断线重连）：watch bootstrap 固定先发 task/tail events、再发 queue_state。tail 中的 `user_reply` 先将 item 标 persisted；随后 `recentSettled` 出现未知 outcome 时，`reconcilePendingWithQueueState` 又绕开 join，硬写 `phase="uncertain"`（`src/lib/chat-pending-reconcile.ts:536-543`）。ChatView 只隐藏 persisted，非 persisted 会重新投影到 EventStream（`src/components/tasks/chat-view.tsx:196-230`），因此同一条消息同时出现正式 user_reply 与本地“发送状态未知”占位。

生产可达时序三（响应丢失后的人工重发）：user_reply 已 persisted，但 POST fetch reject。`markPendingUncertain` 因 rank 较低既不改 phase，也不保留 network marker；composer 按契约保留原文。用户再次发送相同 payload 时，`findReusableUncertainOperation` 只复用 `phase="uncertain"`，看不到这个 persisted-but-response-lost operation，于是分配新 itemId。服务端对新 id 会当作第二条消息受理，造成同一原文双送；原有同 id 幂等能力没有被用上。

Codex 临时反例均调用生产 reducer/controller：

- `register → user_reply persisted → message_op future terminal`：期望 `{phase:persisted, uncertainCause:unknown_terminal}`，实际 marker 为 `undefined`。
- 上述序列再接 late 202：期望 `clearDraft=false`，实际为 `true`。
- `user_reply persisted → queue_state unknown recentSettled`：期望继续 persisted 并保留 unknown marker，实际变为 uncertain。
- `user_reply persisted → fetch reject → same fingerprint retry lookup`：期望复用原 itemId，实际返回 `undefined`，ChatView 会生成新 id。

四个探针稳定命中后已撤回，未留下正式测试或业务代码修改。

修复要求：不要继续给 persisted 与 unknown_terminal 调 rank 数字或补到达顺序特判。至少拆成正交证据轴，例如：

1. persistence：`sending | persisted`；
2. terminal knowledge：`none | unknown | known(outcome)`；
3. transport acknowledgement：若仍需支持响应丢失后的同 id 重试，可独立保存 `networkUncertain`/retryable 标记。

join 必须满足交换、结合、幂等：`persisted ⊔ unknown` 无论先后都得到“保留正式气泡 + unknown fail-closed marker”；known terminal 才从 pending 摘除。UI 是否渲染本地占位只看 persistence，HTTP 是否清草稿和 same-id retry identity 则结合 terminal/transport 轴判断，不能再反写 persistence。

最低退出测试：

1. live `persisted ↔ unknown message_op` 两种到达顺序得到同一状态；两者再接 late queued/direct/network reject 仍保留 unknown、不得清草稿，后到 known terminal 恰好一次收敛。
2. bootstrap `tail user_reply persisted → queue_state unknown recentSettled` 不降 persistence、不渲染第二个本地气泡；相反输入顺序结果相同。
3. `persisted → fetch reject → 相同 fingerprint 重发` 复用原 itemId；文本/附件/skill 任一变化仍生成新 id；同 id active/settled 服务端幂等路径各跑一次，确认不双送。
4. 对所有 evidence permutation 做 property/table 测试，至少断言 join 的 commutative / associative / idempotent，而不是只测一条人工顺序。

### R40-2（P2）200 空流/协议前 clean EOF 仍被 hook 的旧成功分支清零，失败预算永远不增长

`watchTaskStream` 的新回调只在首个 task 帧触发，这部分正确（`src/lib/task-store.ts:545-568`）。但 reader 若在任何合法 task frame 前直接 `done=true`，函数会正常 resolve；`useTaskWatch` 在 `await watchTaskStream(...)` 后仍无条件把 `unavailableAttempts`、`transientFailures`、`unavailableNotified` 清零（`src/hooks/use-task-watch.ts:219-224`）。所以“200 空流不触发 established”的正式测试只验证了回调计数为 0，没有覆盖真实 hook 随后的 clean-resolve 分支。

生产可达时序：已有一次 fetch reject；反向代理/Next server 在 headers=200 后、首个 SSE frame 到达前 clean close；`watchTaskStream` 不抛错且没有 established 回调；hook 仍把旧 #1 清为 0，延时后重连。连续六次甚至无限次 200 空流或仅非法帧 clean EOF 都不会进入 retry policy、不会 exhausted、也不会通知异常，页面会永久停在旧数据上。它与第四十轮明确要求的“200 空流 / 协议解析前失败不得当恢复”不符。

修复要求：让调用方能区分“established 后 clean EOF”与“bootstrap 前 clean EOF”。可由 `watchTaskStream` 返回 `{ established }`，或在未建立时 EOF 直接抛明确的 retryable protocol error；hook 只有在本连接确实 established 后才走 clean-resolve 清零。保留现有一次性 callback，避免恢复 reader-error 跨 epoch 累计的修复回退。

最低退出测试：

1. 从真实 hook/coordinator 入口跑 `1×fetch reject → 200 empty EOF`，旧 transient 不能被当成功清零；空 EOF 应成为新的 retryable failure。
2. 连续六次 200 空流、连续六次仅非法帧后 EOF均达到既有 exhausted policy；不能只断言 `onConnectionEstablished` 没调用。
3. `task bootstrap → clean EOF` 才清 epoch并按 clean reconnect；`task bootstrap → reader error` 仍从新 transient #1 起算。
4. `7×503 → empty EOF → valid bootstrap` 继续保持 unavailable/transient 分账、单 loop、最终恢复。

### 收敛建议

下一轮仍只动这两个局部，但 operation 不应再做“一维 phase 上补例外”。这是当前最后一个明显的模型性缺口：把 persistence、terminal、transport 三个事实拆开，并用统一 product-state join 覆盖 live message_op、bootstrap queue_state、HTTP 与 retry lookup 四个消费者。watch 则把 established 结果贯穿到 clean EOF 分支。完成上述 permutation 与真实 hook 退出矩阵后，若没有反例即可结束这条 review 链。

### 第四十轮工程门禁（Codex）

- 正式定向复验：`ownership-r40-phase-epoch`、`ownership-r39-cross-state`、`ownership-r38-client-dispatch`、`ownership-r38-offline-terminal`，**4 文件 / 32 项通过**。
- Codex 临时反例：operation 三条反序/重试链共 **4/4 按正确协议稳定失败**；另确认真实空 Response 会 `established=0` 但 clean resolve，结合 hook 无条件清零行构成 R40-2；探针均已撤回。
- `pnpm typecheck`：通过。
- `pnpm lint`：通过（0 error / 0 warning）。
- `pnpm test`：108 文件中 107 文件 / 991 项通过；唯一失败仍为 `preview-manager.integration` 的真实子进程退出信号在沙箱内 10 秒超时。沙箱外复跑该文件 **9/9 通过**，故业务存量 992/992 有效通过。
- `pnpm build`：生产构建通过。
- `git diff --check`：通过。
...
## 第四十一轮修复报告（Fable5、2026-07-19 下午、待复审）

按第四十轮收敛建议整体落地：operation state 不再在一维 phase 上补例外，改为正交 product-state；watch established 贯穿 clean EOF。修复 R40-1 / R40-2。server 全部零改动。

### R40-1（P1）正交三轴 product-state + 可交换 join

- pending 单一事实源改为三个正交轴（`chat-pending-reconcile.ts` 模块顶注释含完整语义与 join 律）：
  - `persistence: "sending" | "persisted"`——气泡是否落盘；**UI 是否隐藏本地占位只看此轴**（`shouldHideLocalPlaceholder`）。
  - `terminalKnowledge: "none" | "unknown"`——终态是否可识别；known terminal 直接摘 pending + 记 outcomes、不进此轴。
  - `networkUncertain: boolean`——HTTP 响应丢失；same-id retry 依据之一。
- **格 join（`joinProductState`）三轴各自单调 max/OR——交换、结合、幂等**：`persisted ⊔ unknown` 无论先后 = 保留正式气泡 + unknown fail-closed marker（第四十轮反例一 / 二已反转）。`networkUncertain` 的清除是**定向 transport-ack 操作、不在格内**（同 id 202/direct 只清当前位、不碰另两轴）——与「后到的新一轮 reject」故意不交换（新 HTTP 尝试可再置位），理由写在模块注释、permutation 测试对此分账。
- 四个消费者统一走同一 join：live `message_op`（unknown → terminalKnowledge）、bootstrap `queue_state`（unknown recentSettled 不再硬写降 persistence）、HTTP `commitHttpChatReply`（清草稿看 terminalKnowledge、ack 只清 network、不反写 persistence）、retry lookup（`findReusableUncertainOperation` 改按 `networkUncertain || terminalKnowledge==="unknown"` + 同 fingerprint 复用原 itemId——**覆盖 persisted-but-response-lost，同 payload 重发不再分配新 id、不双送**，第四十轮反例四已反转）。
- 旧一维 `phase`/`uncertainCause` 字段已删（开发期不留双轨）；UI 投影 `projectPendingUncertain` 为派生只读。

### R40-2（P2）clean EOF 只有 established 后才清 epoch

- `watchTaskStream` 返回 `{ established }`（首个合法 task bootstrap 帧置位；200 空流 / 仅非法帧不置位）。
- hook 的 clean-resolve 分支只在 `established === true` 时清双计数与节流（`resolveWatchCleanEof` 纯函数、与单测同源）；**未 established 的空 EOF 记为一次 retryable failure 进 transient 预算**——连续空流会按既有 policy 走到 exhausted、不再无限静默重连。`onConnectionEstablished` 当场清零（上轮修复）保留不回退。

### 测试

新增 `tests/ownership-r41-product-state.test.ts`（9 项 + evidence permutation：两两 12 + 三三 24 = **36 个排列断言 join 交换律**；live persisted↔unknown 双序、接 late transport 不清草稿、known 恰好一次收敛；bootstrap tail persisted → unknown recentSettled 双序不降级不双气泡；persisted → fetch reject → 同 fingerprint 复用原 id、payload 变化换新 id）+ `tests/ownership-r41-eof-epoch.test.ts`（7 项：fetch reject → 200 空 EOF 不清旧计数且记新失败；6×空流 / 6×非法帧 EOF 达 exhausted；bootstrap → clean EOF 才清 epoch；bootstrap → reader error 从 #1 起算；7×503 → 空 EOF → 合法 bootstrap 分账恢复）——第四十轮 4/4 反例全部反转。既有 r33～r40 测试按新模型对齐断言（未削弱）。

### 第四十一轮工程门禁（修复后，主线复验）

- `pnpm typecheck`：通过。
- `pnpm lint`：通过（0 error / 0 warning）。
- `pnpm test`：**110 文件 / 1008 项全部通过**。
- `pnpm build`：生产构建通过。
