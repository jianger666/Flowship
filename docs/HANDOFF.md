...
| Cognition Labs *Don't Build Multi-Agents* | ⭐⭐⭐⭐⭐ | 反 multi-agent 论据、必读 |
...
| 记忆 | (旧) 文件系统 `data/tasks/<id>/` + 单 agent 长会话——已删、未来重建 |
| 流程（Workflow） | (旧) 单 agent + 多 phase chain（spec → plan → build）；当前**未实现**、形态待和用户讨论是否还分段 |
...
- 单 agent、多 phase（不上 multi-agent）
...
1. **Cognition Labs**（Devin 母公司）的 *Don't Build Multi-Agents*：多 agent 串联在生产里 4 大失败模式
2. **Anthropic** *Building Effective Agents*：90% 你叫"agent"的、其实是 workflow + routing + evaluator
...
### 1. 单 agent / 多 phase / 不串联多 agent

**原因**：避免 hand-off 信息丢失（Cognition 文章核心论点）。  
**实现**：用 Cursor SDK 创建一个 `SDKAgent` 实例、跑完整个 phase 后释放、下一个 phase 再起新 agent（**当前阶段每个 phase 独立 agent、不共享会话**——后续 W2 可考虑改成共享会话保留 spec→plan 的连贯上下文）。
...
2. **Cognition Labs *Don't Build Multi-Agents***  
   ...
   核心：multi-agent 在生产中的 4 大失败模式、为什么单 agent + workflow 更稳
