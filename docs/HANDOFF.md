...
- **恐龙快跑等待视图（用户拍板「等推进无聊、左侧放个游戏当 loading」）**：GameKit UI 的 dino-runner（shadcn registry 单文件、吸主题色）装到 `src/components/games/`；artifact 面板 toolbar 左侧加「产物 / 等待」视图切换——当前 action 运行中且无产物自动进游戏、产物到达 toast 轻提示不强制切走、随时可手动切。打磨三轮：主角改像素 T-Rex 剪影 + 障碍改仙人掌（fillRect 点阵、物理零改动）、去容器边框/焦点环与面板融合、`captureGlobalKeys` 全局按键（输入框豁免、切走视图即卸载）。
