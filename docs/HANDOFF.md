...
- **Hero loading（用户点名「通用的好看的全局 loading」、第一稿甘特骨架被否「不一定非是甘特」）**：`LoadingState` 新增 `variant="hero"`——双弧环（主题色渐变弧快转 + 细弧反向慢转 + 中心柔光呼吸）；纯 CSS（globals.css `hero-*` keyframes）、`prefers-reduced-motion` 自动降级；已用于 首页 gate / 看板拉取 / 任务详情 / 对话页 / 设置页 的页面级 loading
