...
3. **agent 跑前 / 跑后跑确定性工具**：typecheck / lint / git hash / hooks 收敛非确定性
4. **MCP 工具白名单 + 任务级黑名单**：限制 agent 能用的副作用源
5. **Skills 沉淀可复用能力**：场景化判断写在 SKILL.md、跨任务共享
6. **events.jsonl 全量记录**：每次 LLM 调用 / 工具调用 / 用户操作落盘可审计
7. **artifact 持久化 + 可回退**：action 产出落 `data/tasks/<id>/actions/<n>-<type>.md`、可 diff 历史
...
- **多角色支持**：`Task.role` 字段、同一 story 跨角色多 task 并行（Cursor / CC 不区分角色）
- **两套 mode**：task（走 action 容器、需求 → MR）+ chat（自由对话、接近 Cursor / CC 轻量交互）
- **task 模式（主线）**：粘飞书 story → plan / build / review / ship 任意推进、Cursor / CC 不做这件事

## 未来路径（V1.x / V2）

- B 端 rules 库（仿 wk-ai-rules 形态、覆盖表单 / 表格 / 权限 / 接口类型）→ build action 注入目标仓库 `.cursor/rules/` 当 cursor rule 用
