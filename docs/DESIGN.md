...
> - **V0.3.4**：context 合并进 plan（实操中分离价值未兑现、用户审 context 跟审 plan 判断点重合）→ 变 `plan → build`
...
>   1. `Task.role: TaskRole`（当前只有 `"fe"`、未来扩 `be / data / mobile / qa`）、agent prompt 按 `{{role}}` 调整视角、同一 story 跨角色多 task 并行（详见 [MULTI-ROLE.md](./MULTI-ROLE.md)）
...
> - **V0.5（review phase 落地、2026-05-18）**：phase 拓扑加 `review`、变为 `plan → build → review`。review 拿 `git diff × 01-plan.md × 02-build.md × contextDocs` 做结构化差值、4 类差异分流（范围扩张 / 收缩 / 实现偏差 / 未完成）、产出含 commit msg / PR body / 飞书评论草稿。同步加 plan 校验前移（plan agent 在 artifact 里写「我的理解 vs 飞书原文」对照）、多 phase 模型选择（settings 默认模型 + 每 phase ack 时可切）、可选「换新 agent」fork（默认不强制、用户决定）。详见 [HANDOFF.md](./HANDOFF.md) 「V0.5」段。
> - **V0.5.1（联测打磨、2026-05-17 ~ 2026-05-18 持续）**：联测 V0.5 时发现 + 修复的一堆边缘 case：
>   1. SDK 1.0.13 工具名全量修正（`read_file` → `read` / `edit_file` → `edit` / `write_file` → `write`、影响所有 prompt / skill / UI / docs）
>   2. revise feedback 永远先 ask_user 复述（用户拍板：不分支、永远弹、不做 feedback 质量判断）
>   3. resume-waiting 真实性检查（`fs.stat` artifact 大小、防 agent 撒谎说「已产出」）
>   4. agent 中间 phase 提前退 run 修复（prompt 加强约束）
...
> - 第 2 节「单 agent vs 每 phase 独立 agent」——依然有效、当前 plan/chat 都走单 agent 长 Run、V0.5 起 ack 时可手动 fork 起新 agent
> - 第 6 节「Prompt 文件化」——**节内描述还是 4 phase**、当前是 3 phase（plan + build + review）、文件名见 `prompts/phase-{1,2,3}-*.md`
> - 第 14 / 15 节是 V1 三步流程（spec → plan → build）的设计推导、**保留作历史参考**
> - 第 16 节「Chat 模式架构」——基本有效、但 keep-alive 机制已改成 V0.3.5 shell + curl long-poll
> - 第 17 节「V0.2 Workflow 架构」——**节标题写 4 phase、实际当前 3 phase**、其余原则有效
> - 详见 `HANDOFF.md` 「V0.5 / V0.5.1 演进」段（最新）
...
- agent 的 `cwd` 是用户业务仓库（`{{repoPath}}`）、不是 ai-flow 项目根
- artifact 文件在 ai-flow 项目下的 `data/tasks/<id>/artifacts/`
- 如果模板里写「写 `artifacts/01-context.md`」、agent 会写到用户仓库下、走丢
- 现在改成「写 `/Users/.../ai-flow/data/tasks/.../artifacts/01-context.md`」、agent 必然写对
...
>
> ⚠️ **V0.3.5 + V0.4 重大改动**（本节正文还停留在 2026-05-11 的描述、阅读时叠加这些变更）：
>
> - **保活机制**：50s keepalive + anti-loop prompt 已删（V0.3.5）、改成 `wait_for_user` 立刻返回 shell 引导、agent 调 `shell` 工具 curl 一条长 HTTP 连接到 `/api/tasks/:id/wait-ack` 等用户消息。`keepaliveCounters` 全局字段已删
> - **`/start-chat` 路由**：V0.4 已删除、启动职责合并进 `/chat-reply`（用户在 UI 输入框发首条消息时后端自动 spawn agent）
> - **chat 自由化（V0.4）**：表单全选填、首条消息直接作为 `chat-runner.buildInitialPrompt(task, skills, firstMessage)` 的第三参数注入、agent 第一次 turn 就回答、不绕 wait_for_user
> - **contextDocs**：chat 模式也 inject `contextDocs`（V0.4 起）、`renderContextDocsSection` 抽到 `src/lib/server/context-docs-prompt.ts`、plan / chat 共用
> - **`feishuUrl` 字段已删**：plan/chat 都用 `feishuStoryUrl`
>
> 下面 16.5 / 16.7 / 16.8 等节里提到 `/start-chat` 路由、`keepaliveCounters`、50s timer、keepalive 文案变体等内容、请理解为「曾经的设计、已被取代」。
...
chat 启动：
  POST /api/tasks/[id]/start-chat
    ...
    ├─ Agent.run(task.repoPath、{ mcpServers: { aiFlowChat, ...userMcp }, model })
...
start-workflow：
  POST /api/tasks/[id]/start-workflow
    ...
    ├─ Agent.create(task.repoPath、{ mcpServers: { aiFlowChat, ...userMcp }, model })
