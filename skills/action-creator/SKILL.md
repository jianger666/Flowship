...
自定义 Action = **skill 挂载壳**：方法论住在 skill 里、壳只带执行参数（产出要求 / 是否新 agent 等）。
...
- **目录名** kebab-case（字母数字和 `-`，如 `perf-audit`）
...
| `summary` | 一句话简介（可选） |
| `extra_skills` | 附加参考 skill（可选） |
| `fresh_agent` | 默认 true；用户明确说续用上下文再传 false |
