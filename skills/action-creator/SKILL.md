...
2. 建 skill：写法规范见 skill-creator、先 read 它；目录名 kebab-case 英文、落到当前工作目录 `<cwd>/<skill名>/SKILL.md`。**产出要求不写进 skill**——那是壳参数、下一步用 `output` 传。凭证 / 系统工具细节不要写死进 skill（skill 要可移植）；运行时系统会注入可用能力（如 GitLab 凭证、内置飞书 CLI）、skill 里写「调 GitLab API 查 xx」这种抽象步骤即可。
