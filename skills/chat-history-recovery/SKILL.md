...
description: >-
  ...
  或你怀疑 agent 进程重启过、当前上下文不完整时、用 `read_file` 读任务的 `events.jsonl`
  把所需的历史事件找回来。
---
...
- `tool_call`：你调过的工具（read_file / grep / shell / 其他 MCP 等）
- `thinking`：你的思考过程
- `info` / `error`：系统消息 / 错误

## 什么时候该 `read_file` 这个文件
...
- 直接调 `read_file`（SDK 内置工具）读那个绝对路径
