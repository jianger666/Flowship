...
description: >-
  处理 ai-flow chat 模式下用户附的图片 / 文件 / 目录。
...
用户在 ai-flow chat UI 里可以**附两类东西**、`wait_for_user` 返回的 text 会拼对应标记段：
...
1. 对**每张图**用 SDK 内置 `read` 工具读它的绝对路径（注意：工具名就叫 `read`、不是 `read_file`）
2. SDK 内置 `read` 工具对图片文件有 vision 处理：
...
1. 这些是**真实存在**的绝对路径（ai-flow 后端已校验）、直接用就行
2. **文件路径**：用 `read` 工具读内容、然后基于内容 + 用户问题回复
3. **目录路径**：先用 `glob` / shell `ls` 看看里面有啥、再决定要不要 `grep` / `read` 进一步探索
...
- 不要质疑路径合不合法、直接读就行（路径是 ai-flow 后端校验过的绝对路径）
