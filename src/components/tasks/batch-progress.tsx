 ...
 * - 没拆批次：不展示入口（分批概念随团队规范逐渐淡化、半兼容——数据与逻辑保留、仅隐藏按钮）
...
export const BatchProgress = ({ task }: Props) => {
  ...
  // 半兼容：未分批不再展示「未分批」入口（不再引导分批、分批概念将随团队规范淡化）；
  // 拆了批次照常显示进度 chip + Dialog。钩子已全部在上方、提前 return 安全。
  ...
  return (
    <>
      {/* 触发 chip：实色「批次进度 N/M」、点开 Dialog 看进度 + 每批详情 */}
      <Button
        type="button"
        variant="outline"
        size="sm"
