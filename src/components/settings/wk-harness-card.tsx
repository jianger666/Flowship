...
/**
 * 「团队 wk 流程」配置节（设置页「团队」分组的整卡内容）
 *
 * 单独成组（用户拍板「团队模块独立」）而不是塞在「连接」的理由：这两项都是**对接团队
 * harness 基建**的参数——Delivery Hub 就是一台团队服务器（和 GitLab / 飞书 / 环境配置同类）；
 * WK 产出目录虽是本机目录、但它和 Hub 写的是同一份 `~/.wk/config.yaml`、属于同一个接入动作，
 * 拆开放会让人配一半忘一半。
