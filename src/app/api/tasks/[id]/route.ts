...
/** 等 Agent.create/send 飞行窗口退出（waitFor* 只等可见 record） */
const DELETE_STARTING_POLL_MS = 100;
const DELETE_STARTING_WAIT_MS = 8_000;

/**
 * 仅物理目录已消失且 refs 无 pending 时才删 journal。
...
const removeDeletionJournalIfFullyDone = async (
  ...
  // manifest 未确认 / journal 读未知 → 绝不删 journal
  const journal = await readDeletionJournal(taskId);
  if (journal.kind === "unknown") {
    console.error(
      `[DELETE /api/tasks/[id]] removeJournal: 读未知、保留 task=${taskId}`,
      journal.error,
    );
    return;
  }
...
/** 已提交删除事务 → 202 recoveryPending（鼓励前滚、禁止 4xx 诱使重试破坏） */
 ...
 * 禁止用空 repoPaths 推断 confirmedEmpty——只信构建结果显式标志。
 */
const prepareDeleteManifest = async (
  taskId: string,
): Promise<CheckpointRefManifest> => {
  // meta 三态快照——unknown 不得折叠成「无仓」
  ...
  if (resolved.ok) {
    ...
    return {
      ...
      phase: "prepared",
      repoPaths,
      // 只传播构建侧 confirmedEmpty，禁止空数组推断
      ...(resolved.manifest.confirmedEmpty ? { confirmedEmpty: true } : {}),
    };
  }
  console.warn(
    `[DELETE /api/tasks/[id]] manifest 未确认 task=${taskId}: ${resolved.error}`,
  );
  return {
    deletedAt: Date.now(),
    checkpointRefs: [],
    ...
    // 仅 valid meta 快照可写入；unknown/absent = 空，禁止随后 rm 最后恢复源
...
export const GET = async (req: Request, { params }: Ctx) => {
  try {
    ...
    if (Number.isFinite(tailParsed) && tailParsed > 0) {
      ...
      // helper 与 Response 之间可插入删除——failpoint + 提交点同步复查
...
export const DELETE = async (_req: Request, { params }: Ctx) => {
  ...
  // 任何 await 之前先占 deleting——收尾窗口内禁止新预约覆盖 cancelled lease。
  ...
  try {
    // 事务入场前三态检查——unknown 不得发布不可逆终态；双 absent 幂等不建 journal
    ...
    if (priorJournal.kind === "unknown" || priorTomb.kind === "unknown") {
      ...
      console.error(
        `[DELETE /api/tasks/[id]] 入场证据读未知、503 不进删除 task=${id}`,
    ...
    if (
      ...
      void recoverDeletedTaskArtifacts(id).catch((e) =>
        console.error(
          `[DELETE /api/tasks/[id]] 已提交重入 recovery 失败 task=${id}`,
          e,
        ),
      );
    ...
    if (taskPresence === "unknown") {
      endChatLifecycle(id, "deleting");
      console.error(
        `[DELETE /api/tasks/[id]] 任务存在性未知、503 不进删除 task=${id}`,
      );
      return NextResponse.json(
        { error: "删除证据暂不可读、稍后重试" },
        { status: 503 },
    ...
    if (
      ...
      // 从未存在 / 已完全收尾 → 幂等 200，不创建 immortal journal
      endChatLifecycle(id, "deleting");
      return NextResponse.json({ ok: true });
    }
    ...
    revokeTaskOps(id);
    // 等 rewind 事务退出后再 clearChatGate / 清 refs / 删目录，
    ...
    // 撤销启动 lease（标 cancelled），占位后、runChatSession 注册前的窗口里
    // DELETE 到达时 owner 能感知并中止（勿只 release——旧请求无从发现）。
    cancelChatStart(id);
    cleanupChatTaskState(id);
    // 清 action 屏障 Map——防 DELETE 后工具 wait 永久挂死泄漏
    ...
    if (isTaskStarting(id) || hasResourceJobs(id)) {
      ...
      if (hasResourceJobs(id)) {
        ...
        if (resourceJoinTimedOut) {
          console.error(
            `[DELETE /api/tasks/[id]] resourceJobs join 超时 task=${id}、已 quarantine；目录删除延迟到 job 归零`,
          );
        }
      }
    }
    // 无飞行消费者时才清 pending——飞行中的启动链需要标记自裁
    if (!isTaskStarting(id)) {
    ...
    if (resourceJoinTimedOut || hasResourceJobs(id)) {
      ...
      // 返回 200 之前必须先形成 durable committed logical delete
      await writeDeleteTombstone(id);
      // 逻辑删除已提交 → 通知既有 watcher 关流（非删除发起 tab 也停）
      publishTaskStreamEvent(id, { kind: "task_deleted", taskId: id });
      void (async () => {
        try {
          ...
          // recover 内按 journal 清 refs；失败写回 refsPending、不假成功删 journal
          await recoverDeletedTaskArtifacts(id);
          clearChatGate(id);
        } catch (err) {
          endChatLifecycle(id, "deleting");
          console.error(
            `[DELETE /api/tasks/[id]] 延迟删除异常 task=${id}`,
            err,
          );
        }
      })();
    ...
    // deleting gate 必须持有到 deleteTask 成功之后——此前 clear 会开闸让
    // chat-reply 在 refs 清理 / 物理删除窗口内重新预约并起新 Agent。
    // 快速路径 prepared → committed（不可逆前）→ refs → rm；
    // committed 之后任何失败只前滚，不再回滚 journal。
    // durable descriptor（repoPaths / confirmedEmpty）须在 commit 前写入 journal
    const fastManifest = await prepareDeleteManifest(id);
    await writeDeletionJournal(id, { ...fastManifest, phase: "prepared" });
    // 进入不可逆 refs/rm 前原子推进 committed
    await commitDeletionJournal(id);
    const committedRead = await readDeletionJournal(id);
    // committed 后读未知 → 503、不 publish 不可逆终态（磁盘可能已 committed，读闸会 unavailable）
    if (committedRead.kind === "unknown") {
      console.error(
        `[DELETE /api/tasks/[id]] committed 后 journal 读未知 task=${id}`,
        committedRead.error,
      );
      endChatLifecycle(id, "deleting");
      return NextResponse.json(
    ...
    // manifestPending → 不按零 ref 清扫；无完整描述则不 rm taskDir
    if (committedJournal?.manifestPending) {
      refsAllSucceeded = false;
      console.warn(
        `[DELETE /api/tasks/[id]] manifestPending、跳过零 ref 清扫 task=${id}`,
      );
      if (!hasDurableDeleteDescriptor(committedJournal)) {
        // 恢复描述不完整——保留 meta/rewind 作恢复源，journal 停 committed+manifestPending
        console.error(
          `[DELETE /api/tasks/[id]] manifestPending 无完整 repoPaths、不 rm taskDir task=${id}`,
        );
        publishTaskStreamEvent(id, { kind: "task_deleted", taskId: id });
        clearChatGate(id);
        return recoveryPendingResponse();
      ...
      if (!refResult.allSucceeded) {
        ...
        console.warn(
          `[DELETE /api/tasks/[id]] cleanupCheckpointRefs 部分失败 task=${id} pending=${refResult.pending.length}`,
        );
      }
    }
    const ok = await deleteTask(id);
    if (!ok) {
      // 已 committed → 前滚；unknown → 503 不 publish
      ...
      if (still.kind === "present" && still.value.phase === "committed") {
        ...
        void recoverDeletedTaskArtifacts(id).catch((e) =>
          console.error(
            `[DELETE /api/tasks/[id]] recovery 后台失败 task=${id}`,
            e,
          ),
        );
        return recoveryPendingResponse();
    ...
    clearChatGate(id);
    // 故意不 clear generation——保留 tombstone 防 ABA（无键默认 0 会让旧 snap 复活）；
    // task id 不复用，Map 留 string+number 可忽略
    // manifest / refs 未收尾 → 对外 accepted + recoveryPending
    ...
    // committed / tombstone → 前滚 + publish；unknown → 503 不 publish
    ...
    if (journal.kind === "unknown" || tomb.kind === "unknown") {
      endChatLifecycle(id, "deleting");
      console.error(
        "[DELETE /api/tasks/[id]] 证据读未知、503 不发布 task_deleted",
        err,
    ...
    if (
      ...
      void recoverDeletedTaskArtifacts(id).catch((e) =>
        console.error(
          `[DELETE /api/tasks/[id]] catch 后 recovery 失败 task=${id}`,
          e,
        ),
      );
      console.error(
