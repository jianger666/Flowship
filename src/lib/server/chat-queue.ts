/**
 * Chat 运行中消息排队（P5.1）+ MessageOperation aggregate
 *
...
/** recentSettled FIFO 上限（断线重连可重放对账） */
export const RECENT_SETTLED_MAX = 50;

/**
 * 清队终态 reason（机器可读，经 queue_failed / message_op 下发前端）。
...
/**
 * ledger 终态 outcome（= 共享 schema MessageOpOutcome）。
...
/** wire 终态 outcome（= 共享 schema，经 QueueItemSettleOutcome 别名） */
...
/**
 * claim 赢家持有的唯一 handle——route 外层 finally 只认三种出口：
 * transfer（queue/runner 接管）/ settle terminal（phase 已终态）/ release（仍 accepting）。
 */
export type MessageOpHandle = {
  readonly itemId: string;
  readonly taskId: string;
  /** queue / runner 已接管——finally 不再 release */
  transfer: () => void;
  /** 登记本批 staged 附件（仅 accepting 有效） */
...
/** watch bootstrap 完整 operation snapshot 条目 */
...
/**
 * 稳定 payload 指纹——委托共享 `computeChatPayloadFingerprint`
...
/** 尽力删除 staged 附件（release 补偿；失败吞掉） */
...
/**
 * 进程内单调短 id，供 202 ↔ pending ↔ queue_failed 对账。
...
export type QueuedChatMsg = {
  /**
   * 稳定队列项 id（入队时分配；202 / pending / queue_failed 对账用）。
...
/**
 * 把 item 记入 per-task recentSettled（已有则跳过；超上限 FIFO 丢最老）。
...
export const recordQueueItemSettled = (
  ...
  if (next.length > RECENT_SETTLED_MAX) {
    ...
    map.set(taskId, kept);
    // 与 recentSettled 同一有界索引淘汰 terminal messageOps
  ...
  // ledger 与 operation 同推进（保留 staged 清单直至终态）
  ...
  // 终态实时下发（纯内存帧）——成功终态（delivered）此前只有 recentSettled
...
/** 只读快照——bootstrap queue_state.recentSettled */
 ...
 * 同 (taskId, clientItemId) 幂等——已 active 返 alreadyAccepted；
...
export type EnqueueResult =
  | {
      ...
      queuedCount: number;
      itemId: string;
      /** 同 id 已在队内/in-flight，幂等命中未再 append */
...
/** 查 recentSettled 中某 item 的终态（无则 undefined） */
...
/**
 * 完整 MessageOperation aggregate 快照（含 accepting/persisted/handedOff/failed）。
...
/** 把本批落盘附件挂到 accepting op（transfer 前可回滚） */
...
/**
 * transfer 成功——清空回滚清单（文件已归 queue / user_reply 持有，不再随 release 删）。
 */
...
const createMessageOpHandle = (
  ...
  return {
    ...
    finalize: () => {
      ...
      // persisted 却未 transfer/settle → 明确失败终态，禁止幽灵 active
...
/**
 * 同步原子 claim——必须在任何附件落盘 / checkpoint / send 之前调用。
 *
 ...
 * - terminal 已被有界淘汰后 → 视作 absent，允许同 id 新受理
 */
...
/**
 * user_reply 落盘后推进到 persisted（非终态）。
...
/**
 * agent 真正接管后 settle handedOff（ledger outcome=delivered）。
...
/**
 * 明确失败终态 + ledger（queue_failed 由 failQueuedItems 统一 publish）。
 */
...
/**
 * 受理后、persisted 前的软失败（409 停止 / 队满等）→ 释放 claim，
...
/** 是否已是真终态（handedOff / failed）——禁止再重排 */
...
/**
 * 按 (taskId, clientItemId) 查幂等受理态（enqueue 防御 / 只读）。
 ...
 * 可选 itemId 一并挂上，供 queue_state bootstrap 对账。
 */
...
export const enqueueChatMessage = (
  ...
  // 客户端预生成 id → 先查 operation / 队内+in-flight / recentSettled
  const clientId =
    typeof msg.itemId === "string" && msg.itemId.trim()
      ? msg.itemId.trim()
      : undefined;
  ...
  // in-flight 占容量，dequeue 窗口内新消息诚实 409，不靠塞回丢尾
 ...
 * 容量已预留 in-flight，正常不会超限；若理论超限仍保留塞回
...
/**
 * 同步取出并清空剩余排队条目的 itemId（不含已 dequeue 的 in-flight 条）。
...
/**
 * 唯一清队终态提交入口（QueueOperation sink）。
 *
 * 契约：
 ...
 *    - 仅 persisted（有气泡未 handoff）→ 进 failed + queue_failed（禁止当 delivered）
...
export const failQueuedItems = (
  ...
  options: {
    ...
    /**
     * 当前条已 handedOff（send===sent / run owner 接管）。
     * 仅此时不算 failed；「有气泡」不够。
     */
    currentHandedOff?: boolean;
    /**
     * @deprecated 旧名误把 persisted 当 delivered。
  ...
  // stop/cancelled 等旁路常不传 currentItemId——自动纳入 in-flight
  const inflightId = inFlightItemIdsMap().get(taskId);
  const currentId = options.currentItemId ?? inflightId;
  // 只有真 handoff 才豁免；persisted 旧标志不再当 delivered
  const handedOff =
  ...
  // clear 仅允许经本 sink（内部私有语义；导出留给 DELETE/测试）
 ...
 *
 * ⚠️ 业务清队必须走 failQueuedItems。本函数仅供：
 * - failQueuedItems 内部
...
/**
 * 删任务收尾：队列 + generation + in-flight + recentSettled 一起清（
 ...
 * 避免 client pending 对账把「已撤回」条当成仍 active。
...
export const removeQueuedChatMessages = (
  ...
  // 选择性出队也要闭环 op ledger（非 failQueuedItems 全清路径）
  for (const m of removed) {
    if (m.itemId && !isMessageOperationTerminal(taskId, m.itemId)) {
      settleMessageFailed(taskId, m.itemId, "cancelled");
    }
  }
  if (removed.length > 0) {
    publish(taskId, {
      kind: "queue_failed",
      itemIds: removed.map((m) => m.itemId).filter(Boolean),
      reason: "cancelled",
    });
  }
  return removed;
