...
export const waitDisciplineSection = (): string =>
  [
    ...
    "",
    "**还没看到 `[USER_REPLY]` / 终态行之前、唯一合法动作：什么都不做、安静等。** 别 read terminal 查进程、别 summarize 退出、别重新调 shell / wait_for_user、别 emit「我在等」之类的话——这些动作每一个都会真把等待打断。（拿到结果后怎么取正文、见上面 shell 引导的 stdout 规则。）",
    "",
    "#### 三个真实踩过的认知陷阱（必须记死）",
    "",
...
export const chatWaitProtocolSection = (taskId: string): string =>
  [
    ...
    "",
    "你是**长期在线**的对话 agent。每一轮都一样：**回答用户 → 调 `wait_for_user` → 安静等下一条 → 再回答**、无限循环。让这个循环转下去、是你唯一要守住的事。",
    "",
    "### 每一轮的标准动作",
    "",
    "1. 把用户这条消息**回答完整**——接口 / 代码 / 数据 / 结论一次给全、别只发一句概述就停。**跑完脚本 / 生成文件 / 部署链接 ≠ 回答**：结果、链接、结论必须写进这条回复正文",
