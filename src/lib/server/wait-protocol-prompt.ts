...
export const chatWaitProtocolSection = (taskId: string): string =>
  [
    ...
    "### 每一轮怎么做",
    "",
    "1. **要查代码 / 查资料才能回答？先查、再答。** 用 `grep` / `read` / `shell` 等把事实查清楚——查的过程里**别**发『正在检索 / 正在处理 / 让我看看』这类预告**当回复**、那**不是**答案。",
    "2. 查清楚后、把**结果 / 代码 / 链接 / 结论写成一条正文消息发出去**——这条用户在对话框能看到的正文、才算「回答」。`调完工具 / 跑完脚本 / 部署完 / 想完了` 都 **≠** 回答（用户看不到工具调用、也看不到你的思考、只看得到你发的正文）。",
...
export const chatShellWaitGuideBody = (url: string): string =>
  [
    ...
    "- `[USER_REPLY] <文本>` = 用户发来新一轮消息：**先读懂并处理这条消息**（要查代码 / 资料就先查）→ 把答案 / 结论写成**一条正文消息**发出去 → **再调 `wait_for_user` 重新挂等**。别只因为看到 `[USER_REPLY]` 就直接再 wait。回答完绝不结束 run——哪怕用户只说「好的 / 谢谢」、或你回复里反问了用户，也照样回一句再挂等（只有 `[CANCELLED]` 才收尾、详见起手「认知陷阱」陷阱二）。",
    "- `[CANCELLED]` = 收尾结束 run。",
