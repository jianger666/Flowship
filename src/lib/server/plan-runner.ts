...
const buildSuperPrompt = async (
  ...
  return [
    ...
    "     **执行步骤（3 步、按顺序）**：",
    "     ",
    "     1. **永远先调 ask_user**（强制、无分支、无例外）：",
    "        ",
    "        ask_user 的 `question` **根据 feedback 内容动态写**：",
    "        ",
    "        - **feedback 清晰明确**（如「字段 X 改只读」「删掉 §6.3」「补一句业务规则 Y」）：",
    "          question：「我理解你的意思是：<把 feedback 翻译成具体改动、含位置 + 改前 + 改后>、对吗？」",
    "        - **feedback 不清楚但有内容**（如「这块不对」「再加点细节」「跟我想的不一样」）：",
    "          question：「你说『<原文>』、我猜你可能是想：<列 2~3 个最可能的解读>。哪个最贴近你的意思？或者你重新说一下？」",
    "        - **feedback 极短 / 含混**（如「test」「111」「啊」「ok」「随便」）：",
    "          question：「你的留言『<原文>』我没看明白、想改本 phase（你正在做的那个、看上面 Phase 列表）的 artifact 哪一部分？你可以选个方向、或者直接重新说。」",
    "          options 根据当前 phase 给「猜测方向」选项、如 plan phase：「需求理解 / 改动清单 / Task 拆分 / 验收标准 / 待澄清项 / 我重新打字说」；build phase：「未完成 task / 改偏的代码 / 漏改的文件 / 校验失败项 / 我重新打字说」；review phase：「整体一致性结论 / 差异分类 / 交付信息 / 我重新打字说」",
    "        ",
    "        ask_user options 模板（按上面分支挑用）：",
    "          * `id=yes`、`label=「是、按这个改」`",
    "          * `id=partial`、`label=「部分对、补充见自由文本」`",
    "          * `id=no`、`label=「不对、我重新说」`",
    "          * （含混分支额外加几个「猜测方向」选项、引导用户做选择）",
    ...
    "     ",
    "     2. **拿 ask_user 答案后**（[USER_REPLY] 解析）：",
    "        - `yes` 或自由文本说 yes/对/可以/嗯/正确 之类 → 进步骤 3 真改",
    "        - `partial` + 自由文本说明 → 合并补充信息、走步骤 1 再问一次确认",
    "        - `no` 或选了「猜测方向」选项 + 自由文本补充 → 把新理解整理出来、走步骤 1 再问一次确认",
    "        - 自由文本仍然含混（又说回了「111」之类）→ 走步骤 1 再问一次、把上轮 question 措辞调整一下、不要原话重发",
    "     ",
    "     3. **改 artifact**：按确认过的理解改 artifact（用 `edit` 工具改已有内容、不是 `write` 整文件覆盖）、改完**立刻再调一次 wait_for_user**（同 phase 同 artifact）",
    "     ",
    "     **绝对禁止**：",
    "     - 拿到 [PHASE_ACK revise] 后**第一个 tool_use 直接是 `edit` / `write` 改 artifact**——这是模型最容易踩的坑、用户会被「闷头改」气死",
    "     - 拿到 [PHASE_ACK revise] 后**直接 emit assistant_message + 再 wait_for_user**（这是 V0.5.1 上一版本的弱设计、已废弃）——任何 revise 都必须先经过 ask_user 弹窗、不能用 emit 跳过弹窗",
    "     - 在 ask_user 拿到「yes」之前就开始改——用户没拍过板、改了也是浪费",
