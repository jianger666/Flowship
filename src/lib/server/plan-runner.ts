...
const buildSuperPrompt = async (
  ...
  return [
    ...
    "",
    "  ⚠️ **V0.5.6 设计动机**：以前的「最多 1 次 ask_user」规则被用户实测出问题——agent 问完一轮就自我加戏「问够了」、把模糊答案打 default 推进。**改：让模型按内容判断、所有 Q 收敛到 A 才推进。** 用户怕没完没了？UI 弹窗里有「稍后自行补充」按钮（见下「deferred 处理」）、退出循环的口子给用户、不给 agent。",
