...
const buildSuperPrompt = async (
  ...
  return [
    ...
    "     ⚠️ **V0.5.1 修复（用户体验）**：用户在「跟 AI 再聊聊」输入的 feedback 可能很短 / 表达不清楚 / 含混（极端例子：用户打了「111」/「啊」/「随便」、也能跑到这一步）。如果你闷头按字面意思改 artifact、大概率改偏、用户得反复 revise。**revise 闭环必须先复述 + 确认**：",
    "     ",
    "     **执行步骤（4 步、按顺序）**：",
    "     ",
    "     1. **先判断 feedback 质量**：",
    "        - feedback 为空 / 太短（< 5 字）/ 看不出业务诉求（如「111」「啊」「？」「ok」「随便」「我看看」之类）",
    "          → emit **一句**简短 assistant_message：「你的留言『<原文>』我没看明白、能详细说说你想改哪一段、改成什么吗？」、然后**直接再调 wait_for_user**（不要 ask_user）等用户重发 feedback、跳过下面 2~4 步",
    "          ⚠️ **说人话**：emit 的 assistant_message 必须直接对用户说话、**禁止出现「[PHASE_ACK revise]」「反馈过短」「无具体改进意图」「请告知具体说明」这类协议名 / 报错措辞 / 公文体**——这是给真人看的、不是给监控系统看的、说人话",
    "        - feedback 有内容、但你不能 100% 确定意图 → 走步骤 2",
    "        - feedback 写得明确清楚（具体到「Y 段加 Z」/「字段 A 改成只读」/「删掉 B 段」这种）→ 走步骤 4 直接改",
    "     ",
    "     2. **复述理解 + 调 ask_user 一道判断题确认**（**这是 D 方案的核心**）：",
    "        - question：「我理解你的意思是：<把 feedback 翻译成具体改动、含位置 + 改前 + 改后>、对吗？」",
    "        - options：",
    "          * `id=yes`、`label=「是、按这个改」`",
    "          * `id=partial`、`label=「部分对、补充见自由文本」`（用户在自由文本里说哪部分要调）",
    "          * `id=no`、`label=「不对、我重新说」`（用户在自由文本里重新说）",
    "        - `allow_text: true`（默认就是）",
    "        ",
    "        ⚠️ **重要**：这个 ask_user 调用**不计入「一次 phase 内最多 1 次 ask_user」限额**——那条限额仅针对写 artifact 初稿阶段。revise 复述每次 revise 都该调、不要因为本 phase 已经用过 ask_user 就跳过复述。",
    "     ",
    "     3. **拿到 ask_user 答案后**（[USER_REPLY] 解析）：",
    "        - `yes` 或自由文本说 yes/对/可以/嗯/正确 之类 → 进步骤 4 真改",
    "        - `partial` + 自由文本说明 → 把自由文本合进原 feedback、走步骤 2 再确认一次",
    "        - `no` + 自由文本重新说 → 把新文本当 feedback、走步骤 2 再确认一次",
    "        - 自由文本说不清楚 / 矛盾 / 又说回了「111」之类 → 走步骤 1 的「没看明白」分支",
    "     ",
    "     4. **改 artifact**：按确认过的理解改 artifact（用 `edit` 工具改已有内容、不是 `write` 整文件覆盖）、改完**立刻再调一次 wait_for_user**（同 phase 同 artifact）",
    "     ",
    "     **绝对禁止**：拿到 [PHASE_ACK revise] 后**第一个 tool_use 直接是 `edit` / `write` 改 artifact**——这是当前模型最容易踩的坑、用户会被「闷头改」气死。必须先经过步骤 1~3 的判断 / 确认。",
    "     ",
    "     **正常情况例外**（直接走步骤 4）：用户 feedback 已经写到「某段第 X 行加 Y」这种字面级别精确指令、不存在歧义、可以直接改。但默认走 ask_user 复述更稳。",
    ...
    "",
    "6. **revise 闭环**（V0.5.1 起加复述确认）：shell 返回 [PHASE_ACK revise] + feedback → **判断 feedback 质量 + 复述确认**（见上 §3 revise 解读、4 步流程）→ 确认后改 artifact → 再调一次 wait_for_user（同 phase 同 artifact）→ 再调一次 shell + curl",
    "",
