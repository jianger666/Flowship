...
export interface UpdateTaskFieldsInput {
  ...
  /**
   * 显式清 / 设 chat 自动标题挂起标记。
   * false → 删除字段（用户手动改名、生成完成、超时放弃）；true 一般只在建任务时写。
   * 注意：首条消息截断改标题（deriveChatTitleFromMessage）不传本字段，
   * 故意保留 pending，让 SDK 生成结果还能覆盖临时截断标题。
   */
 ...
 * 用户中途手动改名后 pending 已清 → 返 null、不覆盖。
 */
export const commitAutoGeneratedChatTitle = async (
  id: string,
  title: string,
): Promise<Task | null> =>
  withTaskLock(id, async () => {
    const meta = await readMetaV06(id);
    if (!meta) return null;
    if (meta.titleAutoPending !== true) return null;
    const t = title.trim();
    if (!t) return null;
    meta.title = t;
    delete meta.titleAutoPending;
    meta.updatedAt = Date.now();
    await writeMeta(meta);
    return await hydrateTask(meta);
  });
