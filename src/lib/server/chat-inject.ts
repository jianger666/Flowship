 ...
 *   - 有存活会话 → sendChatMessage
 *   - agent 忙 → enqueueChatMessage（队满 409）
 *   - 无会话 → bootArgs + runChatSession 冷启动
 *
...
export const handleChatReplyInject = async (
  ...
  const enqueueOrReject = async (): Promise<Response> => {
    ...
    const queued = enqueueChatMessage(task.id, {
      ...
      enqueuedAt: Date.now(),
      // 来源标记（如 feishu）跟着消息走：flush 落 user_reply 时合并进 meta，
      // 否则排队路径会丢 meta.source（直发路径由 persistUserReply 合并）
      extraMeta:
        options.userReplyMetaExtra &&
    ...
    if (getChatQueueCount(task.id) > 0) {
      ...
      try {
        ...
        if (head.skipPersistEvent) {
          ...
          // 队首消息自带的来源标记（如 feishu）一并落进 meta
