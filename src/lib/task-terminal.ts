/**
 * client TaskTerminalCoordinator（sticky deleted）
 *
...
/**
 * 分类上下文——404→deleted 只对「已成功 hydrate 的 watcher」安全。
 ...
 *
 * 两类计数独立——长期 503 后一次 fetch reject 不得立刻 terminate_exhausted。
 */
...
export const resolveWatchReconnectPolicy = (input: {
  ...
  if (input.kind === "unavailable") {
    // 503 活着重试；不推进 transientFailures
...
/**
 * watch 流 clean resolve 后的 epoch 处理（hook 与单测共用）。
 *
...
/**
 * 列表 Provider 注册——commit 时推进 epoch / successfulDeletedIds / 移除行。
 * 返回取消订阅。
 */
...
/**
 * 统一 terminal sink——记 sticky + 通知列表侧。
...
/**
 * 提交闸——已 deleted 的 task 快照不得写入 UI。
