...
export const WHATS_NEW: Record<string, string[]> = {
  ...
  "1.9.22": [
    "Windows 点更新，App 不会再消失了：装在哪个盘、路径有没有空格都会装回原位置；万一哪次没装上，会明确告诉你手动装一次",
    "旧版本升上来的第一次更新会提示手动装一次（去发布页装到原目录就行，数据不受影响），装完这一次，之后的自动更新恢复正常",
    "Windows 第一次打开不再弹“启动超时”：首次启动安全软件扫描慢，会多等一会儿，真的起不来才报错",
    "设置里装飞书 CLI 不再报 EPERM 失败：占用时会自动处理，装完消息桥接自动续上",
  ],
  "1.9.21": [
