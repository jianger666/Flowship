 ...
 *
 * Node 侧实现在 `instrumentation-node.ts`：turbo 会把本文件编进 Edge graph，
 * 静态 import 会把 `process.on` / `node:*` 一起拖进去刷警告。只在 nodejs
 * runtime 动态 import，Edge 编译顶层看不到那些 API。
 */
export const register = (): void => {
  // 只在 nodejs runtime 装：edge runtime 没有完整 process、也不跑 SDK
  if (process.env.NEXT_RUNTIME !== "nodejs") return;

  void import("./instrumentation-node").then((m) => m.registerNode());
};
