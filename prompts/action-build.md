...
> - 旧任务完成情况只能一句话引用（如「Task 2 沿用 build #14」），不要重复粘贴旧的关键实现 / 验收处理。否则用户会误以为这轮又实现了一遍，review 也无法判断本轮增量。
...
> ⛔ 多轮 build 时，不要复制上一轮已经完成的 task 详情。未触及的 task 写一句「沿用 build #N，本轮未改」即可；本轮无代码改动时，本段只列「评估结论」，不要铺旧实现。
>
