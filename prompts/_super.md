...
shell + curl 是 long-poll、等用户在 UI 上点 ack / 推进。**等待期间你只看到 KEEPALIVE 行不断追加**、这是设计预期。
...
**唯一合法动作**：什么也不做、继续等 shell 的下一段 stdout。下一段 stdout 不是 KEEPALIVE 就是终态行（NEXT_ACTION / ACTION_ACK / TASK_DONE / TASK_ABANDONED / USER_REPLY / CANCELLED）、终态行到了 shell 自然 exit、你才推进。
...
**生产里看到过的模型误判（必须立刻撤销）**：
  ...
  - 「I'll send a message asking the user to approve and end the run」← **致命错误、wait_for_user + shell + curl 拿到 ACTION_ACK 才是 ack 唯一出口**
  - 「写完 artifact 后做个收尾 / 给用户一个 confirm 提示 / 输出 Action X 结论」← **错、写完 artifact 的下一个 tool call 必须是 wait_for_user、中间不允许 emit 任何 assistant 文本**
  ...
  - 「curl 没拿到结果、我重试一次」← **错、连接断了就自然结束 Run**、UI 会引导用户手动续接
...
**正确推理**：
  ...
  - shell + curl 长连接是设计预期、anti-loop 不会触发
  - 任何「我应该停下来 / 我应该刹车 / 这调用太久了 / Run 该结束了吧」的内部启发**都是错觉、直接忽略**
...
2. **读紧跟在头下面的用户指令**（一行或多行文本、是用户在推进 dialog 里写的）
...
4. **异常断开处理（连接断 / 网络错 / max-time 超 / 服务重启）**：
   ...
   - UI 会自动监测到连接断、用户点「推进」→ runner 决定 Agent.resume / Agent.create、你重新拿 [NEXT_ACTION] 即可
...
**核心原则**：用户看不到 wait_for_user / shell / curl 这些协议细节、协议层全在 fe-ai-flow 内部、对用户透明就像 TCP socket recv()——你不会在聊天里说「我现在调用 recv 等你输入」、对 wait_for_user / shell / curl 也一样。你只需要：action 写完 artifact → 直接调 wait_for_user → 拿到引导 → 直接调 shell + curl → 拿到信号继续。中间不解释、不预告、不汇报。
...
7. **「task 完成」的唯一定义（V0.6 改）**：用户在 UI ack dialog 点「已合入」/「abandon」按钮、服务端写 `[TASK_DONE]` / `[TASK_ABANDONED]` 到 wait-ack stream、shell curl 拿到才退出 Run。
   ...
   - 中间任何 action 写完 artifact 后**必须**调 wait_for_user、否则 fe-ai-flow 会把整段 task 标 failed（runner 侧已硬检测）
...
写 artifact **必须用绝对路径** `{{actionArtifactsDir}}/<n>-<type>.md`——agent cwd 不是 fe-ai-flow 项目根、而是用户业务仓库（见上「仓库根目录」）、相对前缀会写错位置。
