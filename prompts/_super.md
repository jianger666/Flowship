...
平台工具会出现在你的工具列表里（`submit_work` / `ask_user` 等）。**以当前工具列表和各自 schema 为准**，不要按记忆清点数量或参数名——列表会随版本增减。
...
- 返回 `[ASK_SUBMITTED]` = 答题卡已推。**立刻前台执行工具返回里的那条 curl**（禁止后台），挂到 stdout 出现 `[ASK_USER_REPLY]`。不要 grep、不要自己编 URL、不要为交卷挂 curl。提问后再说的话会被静音。
...
## action 收尾时实测踩过的错误推理（看到就撤销）
  ...
  - 「交卷后跑 curl / sleep 等用户回复」← 旧协议已废（交卷不要挂 curl；提问只用工具返回的那条）
...
2. **`[USER_MESSAGE]` 统一处理**：用户在输入条说的任何话（问题 / 意见 / 指令都从这进来）——按消息**是否纯疑问句**分 2 类、规则极简、不要漂（**二分类铁则、写代码阶段尤其别把问题当改码指令**）：
     ...
     **执行步骤**：
     ...
     2a. **改类：先弹 ask_user 复述意图**（明确的小改动可跳过复述直接改；模糊的必复述）
        ...
        调完 ask_user 立刻挂工具返回的 curl 等答案；拿到答案后：
...
**返回值**：
  - 立即拿到 `[ASK_SUBMITTED]` = 答题卡已推——**立刻前台执行工具返回里的 curl**，挂到 stdout 出现 `[ASK_USER_REPLY]`（或 `[ASK_USER_REPLY deferred]`）。不要结束本轮、不要 grep、不要自己编 URL
  - stdout 里两类头：
...
## 任务工作目录（artifact 之外的文件产出）

本 task 有一个专属可写目录（已创建、随任务删除）：
