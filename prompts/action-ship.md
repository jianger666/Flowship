...
ship action 的目标：把当前 task 所有仓的代码改动 push 到 origin、调 `submit_mr` MCP 工具创 GitLab MR **到该仓的「测试分支」（提测、不是合 master）**、用内置 `meegle` CLI 在 story 评论 @ 测试人员告知 MR 链接、写 ship artifact 记录全过程。
...
**A. 自动探测**（task.feishuStoryUrl 非空时优先走、用内置 `meegle` CLI）

1. 查一次 story 的角色成员：
   - `meegle url decode --url <task.feishuStoryUrl> --format json` 拿 `work_item_id` / `simple_name`（再 `meegle project search` 把 `simple_name` 转权威 `project_key`）
   - `meegle workitem get --work-item-id <id> --project-key <key> --format json` 拿到 `work_item_attribute.role_members[]`（或返回体里同源字段）
2. **过滤所有 role.name 含「测试」二字的 role**（公司项目空间至少有「测试」/「测试负责人」/「QA」三种命名习惯、不要写死单一 role.name）、把每个匹配 role 的 `members[]` 拍平到一个列表
3. 列表里每个 `member.key` 就是 **user_key**（纯数字、§4 @ mention 直接用它、**不要**换成 lark_user_id——带前缀 / 错体系会被拒 `cross tenant` / `no permission`）、`member.name` 中文名、`member.email` 邮箱
...
**C. 探不到时 ask_user 兜底**（包括：task.feishuStoryUrl 为空 / `workitem get` 失败 / A 过滤完列表为空）
...
- 用户填工号 / 用户名 → `meegle user search --user-keys '["工号或用户名",...]' --format json`、取返回的 **`user_key` 字段**（纯数字、**不是** `lark_user_id`）→ `set_feishu_testers({ task_id, action_id, user_keys: [user_key, ...] })`
...
写评论：用内置 `meegle` CLI 的 `comment add`。`content` 里用**正式 mention 块**写 @（不是纯文本 `@中文名`）。
...
- [<repoTailName>][ vN if mrVersion>1] <mr_url>
...
' \
  --notify-user-type user_key \
  --notify-user-list "7227652003395092508" \
  --notify-user-list "7483212744695627804" \
  --format json
```
...
- **@ 的正确姿势（关键、必背）**：`task.feishuTesterUserKeys` 是纯数字 user_key（§2 探测时存的）。
  ...
  - **同时传 `--notify-user-type user_key` + 每个 tester 一个 `--notify-user-list <纯数字 user_key>`**（通知参数当前实测不生效、但保留传参——官方修复通知链路后大概率从这俩开关恢复）
...
> **前置门禁**：同 §4（所有仓 MR 无冲突才做）。**不要**先跑 `meegle auth status` 预检——auth status 瞬态失败（网络抖 / exit 2 / 超时）会被误判成未登录而整步跳过；直接尝试流转、失败按原因分类记进 artifact。

提测 MR 已交、把飞书工作项流转到「提测 / 待测试」类节点、让飞书侧状态跟上：

1. 按注入的飞书项目 skills 里 `sop-transition-node.md` 的三步曲走：`workflow get-node`（看当前节点）→ 从节点信息判断合法流转目标 → `workflow transition`（流转）
...
- 状态: ✅ 已流转到「<节点名>」 / ⏭ 跳过（未找到提测类节点） / ❌ 失败（未登录 / 网络 / 权限不足）+ 原因
