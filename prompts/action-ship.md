...
**A. 自动探测**（task.feishuStoryUrl 非空时优先走、两次 MCP 调用：get_workitem_brief 探测 + search_user_info 转 lark_user_id）
...
3. 列表里每个 `member.key` 是 **user_key**（⚠️ **不是** lark_user_id！别直接拿去 @ / 通知）、`member.name` 中文名、`member.email` 邮箱
4. **必须再调一次 `search_user_info({ user_keys: [各 member.key 或 member.email] })` 把 user_key 转成真正的 `lark_user_id`**（取返回里每个用户的 `lark_user_id` 字段、纯数字）——这步**不能省**：role_members 给的 key 是 user_key、§4 @ / 通知要的是 lark_user_id、两者不是同一个值（实测踩坑：直接拿 user_key 当 lark_user_id 用、@ 出来了但人收不到通知）
5. 列表非空 → 调 `set_feishu_testers({ task_id, action_id, user_ids: [转换后的 lark_user_id, ...] })`（存的是转换后的 lark_user_id、纯数字）、artifact §2 记下名单（中文名 + lark_user_id）
6. 列表为空 → 飞书 story 这边的「测试 / 测试负责人」角色都没人挂 → 走 **C** 兜底
...
- 用户填工号 / 用户名 → 调 `user-feishu-project-mcp.search_user_info({ user_keys: [...] })`、取返回的 `lark_user_id` 字段（纯数字）→ `set_feishu_testers({ task_id, action_id, user_ids: [lark_user_id, ...] })`
