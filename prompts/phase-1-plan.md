...
### 5. 一次性打包 `ask_user`（V0.3.2 用户拍板：所有问题一起问）

写完初稿后、把**所有需要用户拍板的点**收集到一个 `ask_user` 调用里、**一次问完**：

- 入参 `task_id={{taskId}}`、`phase=plan`
- **`questions` 数组**：每条 `{ id, question, options, allow_text }`、把所有不确定点都打包进来
...
- **一个 phase 内最多调用 1 次 ask_user**——不要一个一个调（一个一个调会让用户在 modal 里看不全 / 体验破碎）
- 用户在弹窗里答完所有问题才能继续、agent 拿到 `[ASK_USER_REPLY] Q1/A1 Q2/A2 ...` 拼接好的文本
- **不要因为「能写 default 推进」就不问**——用户希望你问、问完再写确定结论
- **已问过的不重问**：调 ask_user 前看 contextDocs 列表里有没有 `Q: xxx` 同款问题、有就直接用现成答案
- **问的问题必须跟本仓库相关**——纯后端 / 跨端的问题、不要问

用户答完后、把对应位置在 01-plan.md 里更新成确定结论。用户如果在自由文本里写「不清楚 / 你定」、按合理 default 走、对应行末尾加 `> （ack 待澄清：xxx）` 标记。
...
- `[PHASE_ACK revise]` + 后续 feedback 文本 → 按 feedback 修 01-plan.md、再调一次 `wait_for_user`
...
> ⚠️ **重要、不能省略**：把 §1 / §2 / §3 里你提炼的**关键点** 跟 contextDocs 拉到的飞书原文 / PRD / 后端方案做对照、列差异。
> 这一步是 V0.5 review phase 「跟飞书需求对照」的前置——这里把差异前置暴露、用户在 plan ack 时拍板、省得到 review 阶段才发现「plan 偏离了原文」循环回 plan。

**对照规则**：

- **每条对照行**：`<我的理解> ‖ <原文片段（带 doc title + 段位）> ‖ <差异类型>`
- **差异类型**：
  - `一致`：我的理解跟原文重合、信息无丢失（这种条目可省、留少数关键的就行）
...
| 「导出权限校验：admin / leader 才能用」 | 后端方案 §3.2「仅 admin & leader 角色」 | 一致 |

**没有差异时**这一节怎么写：就 1 行「✅ 所有关键点跟 contextDocs 原文一致、无补全 / 偏离 / 缺源」。
**有🚨偏离 / 缺源**：在 ack 弹窗 / artifact 顶部都明显能看到、用户能立刻判断要不要 revise。

> 写不出这一节 = contextDocs 极度缺失或 agent 没真读原文。前者在 §11 「待澄清」标「需要用户补 XX 上下文」、后者**禁止跳过**——这是 V0.5 引入的硬约束、防止 review phase 才发现 plan 偏离飞书原文。

## 2. 验收标准

- <列 story / PRD 给的 acceptance criteria、按编号搬原文>
- ...

## 3. 业务规则 / 文案 / 状态

<上下文里提到的具体业务规则、文案、状态流转。原文搬过来、不要总结。>
...
逐条对应「验收标准」的每条、说明哪个 task 覆盖：

| 验收标准 | 覆盖 task | 验证方式 |
|---|---|---|
| 补升任务展示在列表 | T2 | 列表渲染 + 列定义 |
| 点补升弹出弹窗 | T2、T3 | 按钮 click → YyyDialog 打开 |
| 提交成功有 toast | T4 | 接口 200 后 `useMessage` |

## 8. 自动化校验计划（给 Phase 2 build 跑）
...
（看 package.json 实际填、不要硬编码 pnpm。）

## 9. 关联文档（清单 + 摘要）

| 类型 | 标题 | 来源 | 摘要 |
|---|---|---|---|
| 飞书 story | xxx | <url> | <一句话> |
| 飞书技术方案 | xxx | <url> | <一句话> |
| 设计稿 | Figma | <url> | <一句话> |
| 自由文本 | 开发补充 | 用户提供 | <一句话> |

## 10. 上下文冲突（已通过 ask_user 澄清）
...
## 11. 待澄清 / 不确定项（仅剩 ask_user 也答不上来的）
...
- **原文为主**：业务规则 / 验收标准 / 文案直接搬、不要总结成自己的话（信息保真）
