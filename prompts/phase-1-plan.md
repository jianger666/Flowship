...
- **初稿阶段最多打包一次 ask_user**——不要一个问题一个 ask_user 调（用户在 modal 里看不全 / 体验破碎）
...
### 5.1 拿到 ask_user 答案后按清晰度分级（V0.5.5）
...
**护栏**：判不准就按 C 走、宁可多问一轮也不要打 default 跳过。同 SDK Run 多调 ask_user 不额外计费、不要省。

### 5.2 写完结论后调 wait_for_user

所有 Q 都按 A/B/C/D 处理完、ask_user 不再有可问的、再进入步骤 6 调 wait_for_user。
