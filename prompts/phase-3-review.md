...
- ✅ 拿 01-plan.md「§3 涉及面」「§5 Task 拆分」对照 git diff、列差异
...
| 01 §3「涉及面 / 本仓库改动」 | 对照 git diff 看「计划改的 vs 实际改的」 |
...
- `[PHASE_ACK revise]` + 后续 feedback → 按 super-prompt §3 revise 解读分级（A 明确指令直接执行 / B 明确问直接答 / C 含混 ask_user 复述 / D 带图先 read 图）、明确指令通常是「改回 plan」「修改 plan 描述」「补做 task N」、动对应文件后更新 03-review.md「差异分类对照」段 + 重新跑 git diff 复核、再调一次 `wait_for_user`
...
|---|---|---|---|
| `src/lib/foo.ts` | 新增 utility 函数（+ 24 行）| task 3 用到、plan 漏列 | 加入 plan §3 + §5 task 3 |

（无范围扩张时这段写：「无、build 改动完全在 plan §3 范围内」）

### 2.2 范围收缩（plan 列了、实际没改）

| plan 列的改动 | 实际状况（参考 02-build.md）| 建议处理 |
|---|---|---|
| `src/components/Bar.tsx` 调样式 | build 跑后发现样式已是目标形态、无需改 | 从 plan §3 删 + §5 task 2 加「已无必要」注解 |

（无范围收缩时这段写：「无、plan §3 列的改动全部落实」）
...
（全部完成时这段写：「✅ 所有 plan §5 task 都已完成、详见 02-build.md」）
