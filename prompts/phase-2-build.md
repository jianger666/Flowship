...
**写 02-build.md artifact** → 见 super-prompt「跨 phase 共享规范 §1 artifact 写入工具」（创建用 `write`、改已有用 `edit`、首次写前 `read` 一遍 `artifact-writer` skill）。
...
>
> ⚠️ **「改动文件」字段路径写法**：每个文件**必须**写从仓库根 `{{repoPath}}` 起算的完整相对路径（详见「跨 phase 共享规范 §3」）、不要简写成 basename（如 `studentFollow.vue` ❌ → `apps/.../src/views/follow/studentFollow.vue` ✅）——简写后前端 `looksLikePath` 识别不出、用户在 artifact 里看到的是纯文本不能点跳代码。**同样适用于**下面「## 改动文件清单」表 + 「## 验收对照」表的「改动文件」列 + 「## 修改记录」的「改动文件」字段——artifact 里**任何**文件位置引用都遵守。
...
| 文件 | 类型 | 增 / 删 |
|---|---|---|
| `apps/cp-class-advisor-center/src/api/sc.ts` | 修改 | +12 / -0 |
| `apps/cp-class-advisor-center/src/views/sc/PromoteDialog.vue` | 新增 | +88 |
| `apps/cp-class-advisor-center/src/views/sc/list/Toolbar.vue` | 修改 | +6 / -2 |

## 验收对照

逐条对应 01-plan.md「§5 Task 拆分」每个 task 自带的「验收点」、说明本 phase 是否满足。
（V0.5.5 起 plan 不再有单独的「验收对照」表、验收点直接挂在每个 task 上、build 这边按 task 维度核对。）

| Task | 改动文件 | 验收点 | 实际状态 |
|---|---|---|---|
| T1 promoteTask API | `apps/cp-class-advisor-center/src/api/sc.ts` | 参数符合接口文档 § 1.2 | ✓ 通过 |
| T2 列表展示 | `apps/cp-class-advisor-center/src/views/mainHome/selList.vue` | 列表渲染正常 | ✓ 通过 |
| T3 弹窗 | `apps/cp-class-advisor-center/src/views/sc/PromoteDialog.vue` | 按钮 click → 弹窗打开 | ✓ 通过 |
| T4 提交 + toast | `apps/cp-class-advisor-center/src/views/mainHome/recordModal.vue` | 接口 200 后 toast | ⚠ mock 接口、待联调 |
