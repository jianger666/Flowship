...
- §1「需求理解」+ §2「业务规则 / 文案 / 状态」：业务上下文
- §1.1「我的理解 vs 飞书原文」：plan 阶段已经把跟飞书的差异前置暴露、按用户 ack 的口径走、不要回退到原文
- §3「涉及面 / 本仓库改动」：本 phase 的改动清单
- §4「技术决策」：决策点已经拍板、按结论实施、不要二次质疑
- §5「Task 拆分」：执行顺序 + task 自带的验收点（验收点直接对应每个 task、不再有单独的「验收对照」表）
- §6「上下文冲突」+ §7「待澄清 / 不确定项」：phase 1 用户拍过的口径 / 待 follow-up 项、不要再问一次
...
- `[PHASE_ACK revise]` + 后续 feedback → 按 super-prompt §3 revise 解读分级（A 明确改 → 直接改代码 / B 明确问 → 答疑 / C 含混 → ask_user 复述 / D 带图 → 先 read 图）、改完更新 02-build.md、再调一次 `wait_for_user`
...
| Task | 改动文件 | 验收点 | 实际状态 |
|---|---|---|---|
| T1 promoteTask API | `src/api/sc.ts` | 参数符合接口文档 § 1.2 | ✓ 通过 |
| T2 列表展示 | `selList.vue` | 列表渲染正常 | ✓ 通过 |
| T3 弹窗 | `PromoteDialog.vue` | 按钮 click → 弹窗打开 | ✓ 通过 |
| T4 提交 + toast | `recordModal.vue` | 接口 200 后 toast | ⚠ mock 接口、待联调 |
